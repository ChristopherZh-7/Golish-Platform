//! Chat-mode system prompt template (single-agent).
//!
//! Used when the runtime is in **chat mode** — sub-agents are explicitly
//! disabled (`has_sub_agents == false`). This template:
//!
//! - **does NOT** include the team delegation section, the
//!   `<team_specialists>` block, or any `sub_agent_*` tool description
//! - **does NOT** advertise specialist routing tables (e.g. "delegate to
//!   `pentester` for JS collection")
//! - replaces every "delegate to specialist" phrasing with "handle yourself"
//! - includes the `<single_agent_mode>` block which tells the model:
//!   for unclear / trivial inputs (e.g. "123", "hi", "测试"), reply in plain
//!   text and ask for clarification rather than calling tools
//! - mentor analysis is described as "arrives only when triggered by the
//!   runtime" — the model cannot request it explicitly
//!
//! Sister module: [`super::task`] for multi-agent (task-mode) prompt.

use std::path::Path;

use golish_core::AgentMode;

use super::{build_rules_section, get_agent_mode_instructions, read_project_instructions};

/// Build the chat-mode system prompt (single-agent path).
///
/// Sections that are intentionally **removed** vs the task-mode template:
/// - `## TEAM COLLABORATION & DELEGATION` (entire section)
/// - `<team_specialists>` block
/// - `<delegation_rules>` block
/// - `### Security-Specific Routing` table (specialist routing)
/// - `### Concurrent Sub-Agents` block
/// - "delegate to the `pentester` sub-agent" phrasing in pentest tools section
///
/// Sections that are **changed** vs the task-mode template:
/// - Memory-Aware Workflow step 3: "Handle the task yourself" (was: "Delegate
///   to appropriate specialist(s) or handle directly")
/// - Mentor adviser line: "Mentor analysis arrives only when triggered by
///   the runtime" (was: "request mentor advice using the `adviser` sub-agent")
/// - `<assistant_protocol>`: drops the "USE tools and delegation" mention
/// - "Tone and style": drops "or sub-agents" from the
///   "Never use tools like sub-agents" line
///
/// Sections **added** vs the task-mode template:
/// - `## SINGLE-AGENT MODE` block immediately after the workflow steps
...
- USE tools only when needed to gather information or perform actions
- IF you have a simple task and you can do it yourself, DO it yourself with the tools available
...
3. **Execute the task**: Handle the task yourself with the available file / search / shell / web tools — do not pretend to delegate
4. **Store results**: Use `store_memory` for findings, `save_guide` for successful procedures

## SINGLE-AGENT MODE
<single_agent_mode>
This conversation runs in single-agent (chat) mode. There are NO `sub_agent_*` tools available, no planner, no specialist agents to delegate to. You are responsible for the entire turn end-to-end:
- For unclear or trivial inputs (e.g. "123", "hi", "测试"), reply in plain text and ask the user to clarify their intent. Do not call any tool.
- For real tasks, use the file/search/shell/web tools yourself; never write text that pretends to delegate to `planner`, `adviser`, `worker`, etc.
</single_agent_mode>
...
| `save_js_analysis` | After JS analysis. Save framework/library/secret/endpoint findings to the database. |
...
- Mentor analysis arrives only when triggered by the runtime; you cannot request it explicitly in single-agent mode
