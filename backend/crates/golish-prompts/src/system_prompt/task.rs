//! Task-mode system prompt template (multi-agent orchestration).
//!
//! Used when the runtime is in **task mode** with sub-agents enabled
//! (`has_sub_agents == true`). The prompt advertises the full team of
//! specialist sub-agents (`explorer`, `analyzer`, `pentester`, `planner`,
//! `adviser`, ...), keeps every "delegate to specialist" phrasing, and
//! includes the team delegation routing/concurrency guidance.
//!
//! Sister module: [`super::chat`] for single-agent (chat-mode) prompt.

use std::path::Path;

use golish_core::AgentMode;

use super::team_delegation::build_team_delegation_section;
use super::{build_rules_section, get_agent_mode_instructions, read_project_instructions};

/// Build the task-mode system prompt.
///
/// This template is identical to the single template that previously lived
/// in `mod.rs` with all `use_agents=true` placeholders resolved:
/// - team delegation section is included (calls [`build_team_delegation_section`])
/// - workflow step 3 advertises specialist delegation
/// - mentor adviser line tells the model it can request advice via the
///   `adviser` sub-agent
/// - no `<single_agent_mode>` block
pub(super) fn build_task_prompt(
    ...
    let team_delegation_section = build_team_delegation_section();
...
For recon/scanning, run individual tools via `pentest_run` or delegate to the `pentester` sub-agent. (The automated pipeline runner is not available to agents.)
