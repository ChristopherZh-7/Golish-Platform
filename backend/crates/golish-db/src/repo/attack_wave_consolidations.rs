...
pub async fn consolidate_attack_wave(
    ...
    if let Some(cap_reason) = cap_reason {
        ...
        .await?;
        let mut residual_risk_ids = Vec::with_capacity(accepted.len());
        for (ordinal, delta) in accepted.iter().enumerate() {
            let ordinal =
                i32::try_from(ordinal).map_err(|_| conflict("attack_fact_delta_set_too_large"))?;
            ...
            sqlx::query(
                r#"INSERT INTO attack_wave_consolidation_members (
                       ...
                       source_wave_unit_id,organization_id,residual_risk_id,member_hash
        ...
        let observations = organization_deltas
            ...
            .map(|delta| {
                let observation = serde_json::json!({
                    ...
                    "delta_kind": delta.delta_kind,
                    "fact_delta_id": delta.id,
                    "schema_version": "attack-fact-delta-entry-v1",
                    "source_attempt_id": delta.source_attempt_id,
                    "source_wave_run_id": delta.wave_run_id,
                });
                attack_candidate_work_items::SeedAttackObservation {
                    ...
                    technique: delta.delta_kind.clone(),
