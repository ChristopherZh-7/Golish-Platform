-- Fixed Investigation Asset roles are admitted by the server-owned schedule,
-- not by a model-created stage_worker_request. Keep the generic PentAGI
-- worker contract strict while admitting this one exact server-seeded shape.

ALTER TABLE pentagi_logical_dispatch_receipts
    DROP CONSTRAINT pentagi_logical_dispatch_receipts_check;

ALTER TABLE pentagi_logical_dispatch_receipts
    ADD CONSTRAINT pentagi_logical_dispatch_receipts_check CHECK (
            ...
            SELECT 1
             ...
             WHERE schedule.status='applied'
               ...
               AND item.role=CASE subtask.subtask_ordinal
                    ...
                    WHEN 3 THEN 'adviser'
                    ELSE NULL END
               AND item.stable_key=subtask.label
