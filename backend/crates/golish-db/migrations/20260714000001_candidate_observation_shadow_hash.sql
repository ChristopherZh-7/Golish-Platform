-- Keep the Candidate whole-record shadow rebuild on the same manifest
-- attestation contract as the Rust repository.  Candidate manifests now bind
-- both the typed observation and its declared hash; older installations still
-- have the pre-observation projection from 20260712000013.
...
BEGIN
    ...
    -- post-seal audit trigger this closes the read-to-freeze interval even for
    -- a raw SQL finalizer that did not use the repository evidence lock.
    PERFORM 1
