-- Permit one exact ordinary Company Controller finalizer-only recovery.
-- Producer work, the accepted StageDeliverable and the immutable exhaustion
-- output remain append-only. Only the same server-owned Controller item is
-- re-leased, and only when its checkpoint binds the prior terminal fence,
-- closed sibling barrier and accepted submission. Target Intel keeps its
-- stricter reviewer-owned recovery path.
