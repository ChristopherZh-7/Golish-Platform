-- Extend the Stage WorkItem transition contract with one exact open-plan Vuln
-- formulaic Controller recovery. Existing Target Intel and ordinary finalizer
-- recovery witnesses remain unchanged. The new transition is admitted only
-- after every producer is terminal and the current Vuln coverage denominator
-- contains no pending, partial, or error cell.
...
DECLARE
    ...
    company_finalizer_recovery BOOLEAN := FALSE;
    vuln_formulaic_controller_recovery BOOLEAN := FALSE;
BEGIN
    ...
    company_finalizer_recovery :=
        ...
        AND EXISTS (
            SELECT 1
              ...
              JOIN stage_team_plans recovery_plan
                ON recovery_plan.id=OLD.team_plan_id
               AND recovery_plan.operation_id=operation.operation_id
               AND recovery_plan.stage_execution_id=OLD.stage_execution_id
               AND recovery_plan.stage_run_unit_id=OLD.stage_run_unit_id
               AND recovery_plan.scope_snapshot_id=OLD.scope_snapshot_id
               AND recovery_plan.organization_id=OLD.organization_id
              JOIN stage_run_units unit
                ON unit.id=recovery_plan.stage_run_unit_id
               AND unit.operation_id=recovery_plan.operation_id
               AND unit.stage_execution_id=recovery_plan.stage_execution_id
               AND unit.organization_id=recovery_plan.organization_id
               AND unit.status='running'
               AND unit.stage_kind=recovery_plan.stage_kind
              JOIN stage_worker_runs controller
                ON controller.id=recovery_plan.final_submitter_worker_run_id
               AND controller.work_item_id=OLD.id
               AND controller.operation_id=recovery_plan.operation_id
               AND controller.stage_execution_id=recovery_plan.stage_execution_id
               AND controller.stage_run_unit_id=recovery_plan.stage_run_unit_id
               AND controller.organization_id=recovery_plan.organization_id
               AND controller.status='running'
               AND controller.terminal_at IS NULL
               AND controller.lease_token IS NOT NULL
               AND controller.lease_expires_at>NOW()
              ...
              JOIN stage_deliverable_submissions submission
                ON submission.id=(controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,deliverable_submission_id}')::uuid
               AND submission.operation_id=recovery_plan.operation_id
               AND submission.stage_execution_id=recovery_plan.stage_execution_id
               AND submission.stage_run_unit_id=recovery_plan.stage_run_unit_id
               AND submission.organization_id=recovery_plan.organization_id
               AND submission.worker_run_id=controller.id
               AND submission.stage_kind=recovery_plan.stage_kind
               AND submission.attempt_epoch IS NOT NULL
               AND submission.attempt_epoch<=(controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,source_attempt_epoch}')::bigint
               AND submission.lease_token IS NOT NULL
               AND submission.payload_sha256=(controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,payload_sha256}')
             ...
             WHERE operation.operation_id=OLD.operation_id
               ...
               AND operation.current_stage=recovery_plan.stage_kind
               AND operation.runtime_memory_contract='v2_only'
               AND recovery_plan.stage_kind<>'target_intel'
               AND recovery_plan.requests_closed_at IS NOT NULL
               AND recovery_plan.dispatch_epoch=(controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,expected_dispatch_epoch}')::bigint
               AND controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,expected_manifest_hash}'
                    ~ '^sha256:[0-9a-f]{64}$'
               AND recovery_plan.final_submitter_kind='worker'
               AND recovery_plan.aggregator_kind='worker'
               AND recovery_plan.aggregator_role=recovery_plan.leader_role
               AND recovery_plan.dynamic_request_policy->>'coordination_mode'='company_controller'
               AND controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,schema_version}'='1'
               AND controller.attempt_epoch=(controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,source_attempt_epoch}')::bigint+1
               AND controller.checkpoint_version=(controller.checkpoint #>>
                    '{_runtime_company_finalizer_recovery,source_checkpoint_version}')::bigint+1
               AND (SELECT COUNT(*) FROM stage_worker_outputs exact_output
                     WHERE exact_output.team_plan_id=recovery_plan.id
                       AND exact_output.work_item_id=OLD.id
                       AND exact_output.worker_run_id=controller.id)=1
               AND NOT EXISTS (
                    SELECT 1 FROM stage_work_items required_item
                     WHERE required_item.team_plan_id=recovery_plan.id
                       AND required_item.required_for_barrier=TRUE
                       AND required_item.status NOT IN ('completed','exhausted')
               )
               AND NOT EXISTS (
                    SELECT 1 FROM stage_work_items required_item
                     WHERE required_item.team_plan_id=recovery_plan.id
                       AND required_item.required_for_barrier=TRUE
                       AND NOT EXISTS (
                           SELECT 1 FROM stage_worker_outputs required_output
                            WHERE required_output.team_plan_id=recovery_plan.id
                              AND required_output.work_item_id=required_item.id
                       )
               )
               AND NOT EXISTS (
                    SELECT 1 FROM stage_worker_runs live_worker
                     WHERE live_worker.stage_run_unit_id=recovery_plan.stage_run_unit_id
                       AND live_worker.id<>controller.id
                       AND live_worker.status IN (
        ...
        AND EXISTS (
            SELECT 1
              ...
              JOIN stage_run_units unit
               ...
               AND unit.started_at IS NOT NULL
              JOIN stage_worker_runs controller
                ON controller.work_item_id=OLD.id
               AND controller.operation_id=recovery_plan.operation_id
               AND controller.stage_execution_id=recovery_plan.stage_execution_id
               AND controller.stage_run_unit_id=recovery_plan.stage_run_unit_id
               AND controller.organization_id=recovery_plan.organization_id
             ...
             WHERE operation.operation_id=OLD.operation_id
               AND operation.superseded_by IS NULL
               AND operation.current_stage='vuln_triage'
               AND operation.runtime_memory_contract='v2_only'
               AND recovery_plan.stage_kind='vuln_triage'
               AND recovery_plan.requests_closed_at IS NULL
               AND recovery_plan.final_submitter_worker_run_id IS NULL
               AND recovery_plan.final_submitter_kind='worker'
               AND recovery_plan.aggregator_kind='worker'
               AND recovery_plan.aggregator_role=recovery_plan.leader_role
               AND recovery_plan.dynamic_request_policy->>'coordination_mode'='company_controller'
               AND recovery_plan.dynamic_request_policy->>'formulaic_worklist_executor'='vuln_v1'
               AND controller.checkpoint #>>
                    '{_runtime_vuln_formulaic_controller_recovery,schema_version}'='1'
               AND controller.attempt_epoch=(controller.checkpoint #>>
                    '{_runtime_vuln_formulaic_controller_recovery,source_attempt_epoch}')::bigint+1
               AND controller.checkpoint_version=(controller.checkpoint #>>
                    '{_runtime_vuln_formulaic_controller_recovery,source_checkpoint_version}')::bigint+1
               AND OLD.row_version=(controller.checkpoint #>>
                    '{_runtime_vuln_formulaic_controller_recovery,source_work_item_row_version}')::bigint
               AND controller.checkpoint #>>
                    '{source_terminal_checkpoint,stage_team_execution_failure,code}'=
                    'stage_team_worker_lease_expired'
               AND (controller.checkpoint #>>
                    '{_runtime_vuln_formulaic_controller_recovery,terminal_cell_count}')::bigint>0
               AND (controller.checkpoint #>>
                    '{_runtime_vuln_formulaic_controller_recovery,terminal_cell_count}')::bigint=(
                    SELECT COUNT(*) FROM technique_outcomes outcome
                     WHERE outcome.organization_id=recovery_plan.organization_id
                       AND outcome.run_id=recovery_plan.operation_id::TEXT
                       AND outcome.collected_at>=unit.started_at
               ...
               AND NOT EXISTS (
                    SELECT 1 FROM technique_outcomes outcome
                     WHERE outcome.organization_id=recovery_plan.organization_id
                       AND outcome.run_id=recovery_plan.operation_id::TEXT
                       AND outcome.collected_at>=unit.started_at
                       AND outcome.outcome IN ('pending','partial','error')
               )
               AND (SELECT COUNT(*) FROM stage_worker_runs exact_controller
                     WHERE exact_controller.work_item_id=OLD.id)=1
               AND (SELECT COUNT(*) FROM stage_worker_outputs exact_output
                     WHERE exact_output.team_plan_id=recovery_plan.id
                       AND exact_output.work_item_id=OLD.id
                       AND exact_output.worker_run_id=controller.id)=1
               ...
               AND NOT EXISTS (
                    SELECT 1 FROM stage_work_items required_item
                     WHERE required_item.team_plan_id=recovery_plan.id
                       AND required_item.required_for_barrier=TRUE
                       AND (SELECT COUNT(*) FROM stage_worker_outputs required_output
                             WHERE required_output.team_plan_id=recovery_plan.id
                               AND required_output.work_item_id=required_item.id)<>1
               )
               AND NOT EXISTS (
                    SELECT 1 FROM stage_worker_runs live_worker
                     WHERE live_worker.stage_run_unit_id=recovery_plan.stage_run_unit_id
    ...
    IF NOT (
        ...
        OR company_finalizer_recovery
        OR vuln_formulaic_controller_recovery
    ) THEN
