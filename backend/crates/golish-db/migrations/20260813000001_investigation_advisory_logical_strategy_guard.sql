-- Forward correction: the PreparedAction compiler freezes the model-authored logical
-- strategy_id as strategy_decision_id. The original guard incorrectly compared it
-- to the host-owned strategy_artifact_id, making every valid advisory apply fail.
-- All other compound authority and hash checks remain unchanged.
