...
/// 1. **Global policy** (`~/.golish/tool-policy.json`) - User's default preferences
/// 2. **Project policy** (`{workspace}/.golish/tool-policy.json`) - Project-specific overrides
///
...
pub struct ToolPolicyManager {
    ...
    /// Global config (from ~/.golish/)
    global_config: RwLock<Option<ToolPolicyConfig>>,
    /// Project config (from workspace/.golish/)
...
impl ToolPolicyManager {
    ...
    ///
    /// Loads policies from both global (~/.golish/tool-policy.json) and
    /// project ({workspace}/.golish/tool-policy.json) locations, merging them
    ...
    /// Get the path to the global policy file (~/.golish/tool-policy.json).
    ...
    ///
    /// This saves changes to the project-level policy file (`{workspace}/.golish/tool-policy.json`).
    ...
    /// Save configuration to global policy file (~/.golish/tool-policy.json).
