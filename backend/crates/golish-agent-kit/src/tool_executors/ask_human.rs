...
pub async fn execute_ask_human_tool(
    ...
    pending_approvals: &tokio::sync::RwLock<
        std::collections::HashMap<
            String,
            tokio::sync::oneshot::Sender<golish_core::hitl::ApprovalDecision>,
        >,
    >,
    ...
    tracing::info!(
        "[ask_human] Waiting for user response: id={}, type={}",
        request_id,
        input_type
    );
    ...
    match tokio::time::timeout(std::time::Duration::from_secs(ASK_HUMAN_TIMEOUT_SECS), rx).await {
        Ok(Ok(decision)) => {
