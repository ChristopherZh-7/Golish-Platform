//! Pluggable state-snapshot persistence. Vendored from metalcraft
//! (`rust4ai/metalcraft`, MIT) · see `mod.rs` for provenance.
...
impl<S: Reducer> Checkpointer<S> for MemoryCheckpointer<S> {
    async fn save(&self, thread_id: &str, state: &S, next_node: &str) -> Result<()> {
        self.inner.lock().await.insert(
            thread_id.to_string(),
            (state.clone(), next_node.to_string()),
        );
        Ok(())
    }
