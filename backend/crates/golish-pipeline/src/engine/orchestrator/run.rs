//! Core DAG execution loop for one pipeline run. Extracted verbatim from
//! `orchestrator/mod.rs` to keep each file within the size budget; behavior is
//! unchanged. Re-exported as `orchestrator::execute_pipeline_inner`.

use std::sync::atomic::Ordering;

use uuid::Uuid;

use super::super::steps::run_single_step;
...
pub(crate) async fn execute_pipeline_inner(
    ...
    if total_stored > 0 {
        if let Some(emitter) = runner.emitter {
            emitter.emit(
                "targets-changed",
                &serde_json::json!({
                    "source": "pipeline",
