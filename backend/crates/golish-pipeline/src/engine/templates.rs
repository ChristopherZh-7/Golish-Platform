//! Built-in + user-defined pipeline templates, plus the topo sort and
//! condition evaluator shared between orchestrator and step executors.
...
///
/// Tauri command `pipeline_cancel` (in the main crate) flips this to `true`.
pub static PIPELINE_CANCELLED: AtomicBool = AtomicBool::new(false);

pub fn pipeline_from_json(json: &str) -> Option<Pipeline> {
...
pub(super) async fn resolve_port_targets(
    pool: &sqlx::PgPool,
    target: &str,
...
pub fn recon_basic_template() -> Pipeline {
    struct StepDef {
        id: &'static str,
        name: &'static str,
