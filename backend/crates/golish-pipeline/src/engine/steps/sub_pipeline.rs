//! Sub-pipeline step executor: recurses into `execute_pipeline_inner`
...
use super::super::orchestrator::{execute_pipeline_inner, PipelineRunner};
use super::super::types::{
    emit_pipeline_event, PipelineEvent, SingleStepResult, StepResult, MAX_NESTING_DEPTH,
};
use super::resolve_sub_pipeline;

pub(in super::super) async fn run_sub_pipeline_step<'a>(
    ...
    let sub = match resolve_sub_pipeline(step) {
        ...
        None => {
            let msg = format!(
                ...
                step.sub_pipeline.as_deref().unwrap_or("?")
            );
            emit_pipeline_event(
                runner.emitter,
                &PipelineEvent {
    ...
    let sub_result = execute_pipeline_inner(runner, &sub, target, project_path, depth + 1).await;
