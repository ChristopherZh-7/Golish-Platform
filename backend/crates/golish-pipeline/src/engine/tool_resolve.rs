//! Resolve a bare tool name (or command template) to the full shell command
//! by consulting the user's toolsconfig directory.
...
pub(super) async fn resolve_tool_command(
    ...
    let Some(val) = find_tool_json(bare_cmd) else {
