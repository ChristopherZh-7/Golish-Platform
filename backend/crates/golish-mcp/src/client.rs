...
/// Returns the path to ~/.golish/mcp-logs.log, creating the directory if needed.
async fn mcp_log_file() -> Option<std::path::PathBuf> {
    let home = dirs::home_dir()?;
    let golish_dir = home.join(".golish");
    if !golish_dir.exists() {
        if let Err(e) = tokio::fs::create_dir_all(&golish_dir).await {
            tracing::warn!("Failed to create ~/.golish directory: {}", e);
            return None;
        }
    }
    Some(golish_dir.join("mcp-logs.log"))
}
...
async fn connect_stdio(
    ...
    // Spawn a background task to read stderr and write to ~/.golish/mcp-logs.log
