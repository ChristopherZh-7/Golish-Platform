...
///
/// Honors the `GOLISH_PROJECTS_DIR` env var when set so tests can redirect
/// reads/writes into a temporary directory without colliding with the
/// user's real `~/.golish/projects/`.
pub fn projects_dir() -> PathBuf {
    if let Ok(custom) = std::env::var("GOLISH_PROJECTS_DIR") {
        if !custom.is_empty() {
...
pub async fn save_project(project: &ProjectConfig) -> Result<()> {
    ...
    crate::file_storage::init_project_dirs(&project.root_path).await?;
    crate::file_storage::init_project_json(&project.root_path, &project.name).await?;
