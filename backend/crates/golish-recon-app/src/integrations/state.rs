...
/// [`golish_app_core::GolishError`]. This is kept in `state.rs` so both
/// commands can reuse one canonical mapping.
pub fn map_err(e: IntegrationError) -> golish_app_core::GolishError {
    match e {
        IntegrationError::Validation(m) => golish_app_core::GolishError::Validation(m),
        IntegrationError::SchemaNotFound(m) => golish_app_core::GolishError::NotFound(m),
        IntegrationError::ExternalFileCorrupt { path, reason } => {
            golish_app_core::GolishError::Internal(format!(
                "external file corrupt at {path}: {reason}"
            ))
        }
        IntegrationError::Io(e) => golish_app_core::GolishError::Io(e),
        IntegrationError::Yaml(e) => golish_app_core::GolishError::Internal(format!("yaml: {e}")),
        IntegrationError::Json(e) => golish_app_core::GolishError::Json(e),
        IntegrationError::Internal(m) => golish_app_core::GolishError::Internal(m),
