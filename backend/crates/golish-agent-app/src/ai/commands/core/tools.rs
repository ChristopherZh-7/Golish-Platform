...
pub async fn list_sub_agents(
    state: State<'_, AgentState>,
) -> Result<Vec<SubAgentInfo>, GolishError> {
    let bridge_guard = state.ai_state.get_legacy_bridge().await?;
    let bridge = bridge_guard.as_ref().unwrap();
...
#[tauri::command]
pub async fn shutdown_ai_agent(state: State<'_, AgentState>) -> Result<(), GolishError> {
...
#[tauri::command]
pub async fn is_ai_initialized(state: State<'_, AgentState>) -> Result<bool, GolishError> {
