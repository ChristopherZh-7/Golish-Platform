...
//!
//! Lives in `golish-app-core` (L5) so every per-domain app crate
//! (`golish-recon-app`, …) and the main `golish` binary share one copy.
...
pub fn ensure_scoped_mutation(rows_affected: u64) -> Result<(), GolishError> {
    if rows_affected == 0 {
...
/// reads.
pub fn ensure_scoped_found<T>(row: Option<T>) -> Result<T, GolishError> {
