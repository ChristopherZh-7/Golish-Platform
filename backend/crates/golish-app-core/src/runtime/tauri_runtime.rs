...
use crate::pty_interactive::PtyOutputTap;
...
impl TauriRuntime {
    /// Build a Tauri-backed runtime.
    ///
    /// `pty_output_tap` feeds terminal output to AI tool subscribers; callers
    /// pass it from their managed state (the main app's `AppState` /
    /// `AgentState`). It lives in `golish-app-core`, which sits below the
    /// crates that define those managed states, so the tap is injected rather
    /// than fetched here.
    pub fn new(app_handle: AppHandle, pty_output_tap: Option<Arc<PtyOutputTap>>) -> Self {
        Self {
            app_handle,
...
struct AiEventPayload<'a> {
    session_id: &'a str,
    #[serde(flatten)]
    event: &'a golish_core::events::AiEvent,
}
