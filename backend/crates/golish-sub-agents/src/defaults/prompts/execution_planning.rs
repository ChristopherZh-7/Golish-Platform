...
- Port scan: naabu / masscan / nmap (top ports) via pentest_run — discover open TCP ports per concrete IP/CIDR-discovered host only. Use batch list input for chunks of IPs/CIDRs instead of one foreground command per asset: `naabu` with `args="-list {{input_file}} -top-ports 1000 -s c -silent"` plus `input_lines=[...]`; `masscan` with `args="-iL {{input_file}} -p <ports> --rate <safe-rate>"` plus `input_lines=[...]`; `nmap` with `args="-iL {{input_file}} --top-ports 100 -T3"` or stdin-capable `-iL -`. Never feed unresolved domains or URL strings to port scanners.
...
- katana supplement: run katana ONCE over the same web-root list via pentest_run(tool_name="katana", args="-list <urls-file> -jc -silent ...") to harvest extra URLs/endpoints. Its output lands in api_endpoints(source='crawler') and merges/dedupes with browser + js_extract rows automatically (unique on target_id+url+method). katana is a SUPPLEMENT — it does not replace the browser closure crawl, and it must go through pentest_run (never call the CLI name as a direct function).
- Seed normalization: merge browser requests, JS endpoints, crawler (browser + katana) URLs, HTML links/forms, and well-known docs paths into scoped same-origin path seeds. Normalize route templates and query parameter names before probing.
- Directory / path discovery (GOLISH-ENUM-DIR): after JS/API rows have landed, run route_probe_paths in batch (targets=[{target_id, base_url}, ...]). Observed paths are optional because the tool reads target-bound api_endpoints and existing directory_entries from DB by default; it uses wordlist_path, workspace/1.txt, or the built-in fallback wordlist and always runs the full de-duplicated local/built-in wordlist plus recursive queue. It probes parent prefixes, recursively expands verified wordlist directory hits, checks positive statuses against a random baseline to reject soft-404/uniform error pages, respects same-origin/rate limits, and persists only verified positives/auth walls as absolute directory_entries with target_id. Do not promote `rejected_candidates` by hand. `queue_completed=true` means the generated and recursive queue drained. Do not call external directory tools such as ffuf / gobuster / feroxbuster / dirb / dirsearch in enumeration.
- Parameter discovery (GOLISH-ENUM-PARAM): derive parameter names from observed browser requests, JS endpoints, crawler (browser + katana) URLs with query strings, HTML forms, and targeted js_extract_apis param_hints. Do not run active parameter brute-force by default.
- Tool boundary: browser_collect_js_api, js_collect, js_extract_apis, and route_probe_paths are direct AI tools (batch via target_urls / targets). Directory discovery must use route_probe_paths; do not call ffuf, gobuster, feroxbuster, dirb, or dirsearch in enumeration. CLI crawlers such as katana must be called through pentest_run(tool_name=..., args=...) and used only as bounded URL sources. Do not attempt to call a CLI tool name as a direct function.
...
- Recommended batch flow over the pending worklist set: (1) browser_collect_js_api(target_urls=[all pending roots]) -> GOLISH-ENUM-JS + GOLISH-ENUM-JSAPI; (2) katana once via pentest_run(-list) as supplement; (3) js_extract_apis(target_urls=[all roots]) -> GOLISH-ENUM-JSAPI + GOLISH-ENUM-PARAM; (4) route_probe_paths(targets=[{target_id, base_url} for all roots]) -> GOLISH-ENUM-DIR. After each batch call, re-check stage_worklist_status / stage_worklist_next; do not assume a work_item is complete from prose.
