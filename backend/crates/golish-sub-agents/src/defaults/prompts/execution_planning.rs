...
- For EVERY in-scope asset, drive each applicable technique to a terminal coverage cell: LIVENESS (httpx or open-port evidence), PORT (fresh port scan for domain/IP/CIDR-discovered IP), SERVICE-FINGERPRINT (service/version on every open port). Bare URL assets keep LIVENESS only; host/IP targets carry PORT/SERVICE. A MISSING (asset x technique) cell counts as not_attempted and FAILS the gate.
- Coverage cells are evidence contracts: found and checked_empty MUST cite real evidence_refs from the tool run; blocked/not_applicable MUST include a concrete note. If you explicitly submit coverage, fill denominator fields: liveness tested_units=1,total_units=1; port scan tested_units/total_units = scanned port denominator; service fingerprint tested_units = open ports fingerprinted and total_units = open ports discovered. If no ports are open, mark SERVICE-FINGERPRINT not_applicable with a note instead of checked_empty total_units=0.
