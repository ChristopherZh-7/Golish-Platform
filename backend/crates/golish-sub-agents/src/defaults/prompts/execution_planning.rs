...
- Run each technique ONCE per service; for slow fuzzing pass background:true to fan out in parallel (or let the soft timeout auto-background). Either way the result is auto-delivered when it finishes — do NOT poll in a loop or re-run the same command. submit_stage_deliverable waits for attributed background jobs before grading; if it still reports running jobs, wait for their completion notes and resubmit. If a backgrounded scan is clearly stuck (check_job shows it running with no new output for a long time — e.g. a hung DNS AXFR), kill_job it and move on instead of waiting it out.
