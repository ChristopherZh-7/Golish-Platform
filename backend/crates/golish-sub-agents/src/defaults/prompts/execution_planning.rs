...
- Web stack fingerprint: after HTTP liveness or nmap confirms an HTTP(S) endpoint, call eas_fingerprint_web_stack(target_urls=[absolute http(s) URLs]) to enrich web technologies via WhatWeb. This wrapper is web-only: never use it for SSH/MySQL/SMTP/non-HTTP service gaps, and do not call raw whatweb or pentest_run directly. If multiple domains/vhosts resolve to the same IP:port, nmap the IP:port once for service/version, but run WhatWeb once per confirmed web origin (`scheme://host:port`) because Host/SNI can expose different stacks.
