...
- Directory / path discovery (GOLISH-ENUM-DIR): run route_probe_paths over observed JS/API path prefixes and a small local wordlist when available (for example workspace/1.txt or an explicit wordlist_path). It de-duplicates observed paths, probes each parent prefix with bounded requests, optionally recurses one level from verified wordlist hits, checks positive statuses against a random baseline to reject soft-404/uniform error pages, respects same-origin/rate limits, and persists only verified positives/auth walls as absolute directory_entries with target_id. Do not promote `rejected_candidates` by hand. Do not call external directory tools such as ffuf / gobuster / feroxbuster / dirb / dirsearch in enumeration.
...
- Data landing: browser_collect_js_api/js_extract_apis land JS files, API endpoints, parameter names, candidate secrets/configs, frameworks/libraries and route seeds; route_probe_paths lands directory_entries. Use query_target_data/check_stage_asset_coverage to read DB truth after landing.
...
- For EVERY alive web service, use this visible order: (1) browser JS/API collection and saved JS landing, (2) JS extraction of endpoints/routes/observed parameter names/redacted secrets/frameworks/libraries + seed normalization, (3) route_probe_paths over observed prefixes with the small local wordlist when available, (4) parameter coverage from observed requests/query strings/forms and targeted param_hints, (5) check_stage_asset_coverage, (6) slim submit.
