...
- CIDR / netblock seeds: a `type=cidr` seed is a RANGE, not a single host — sweep it with eas_discover_ports to discover concrete live hosts. The guarded output store registers each in-range IP as a child target with CIDR provenance before later fingerprinting; do not register it by assertion. Do NOT httpx a whole /N as one host. Actively scanning a whole netblock is heavier/noisier and is gated by human_approval (D1) — request approval before sweeping a large range.
- Service / version fingerprint: call eas_fingerprint_services(targets=[concrete IPs], ports=[confirmed open ports]) only after port evidence exists. Every confirmed open port, including ports discovered later in this stage, must get exactly one targeted fingerprint attempt unless a concrete blocker makes it impossible. The backend owns the `nmap -sV -Pn -iL ... -p ... -T3` recipe and rejects domains, URLs, and CIDR ranges. Do not infer service from a port number or from HTTP liveness alone.
