...
- Run each technique ONCE per host; for slow scans pass background:true to fan out in parallel (or let the soft timeout auto-background). Either way the result is auto-delivered when it finishes — do NOT poll in a loop or re-run the same command.
...
- Run each technique ONCE per service; for slow fuzzing pass background:true to fan out in parallel (or let the soft timeout auto-background). Either way the result is auto-delivered when it finishes — do NOT poll in a loop or re-run the same command.
