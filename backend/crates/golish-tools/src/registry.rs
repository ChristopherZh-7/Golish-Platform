...
use golish_shell_exec::RunPtyCmdTool;

// Import web/Tavily tools from extracted crate
use golish_web::TavilyState;
...
impl ToolRegistry {
    ...
    pub async fn with_config(workspace: PathBuf, config: ToolRegistryConfig) -> Self {
        ...
        let tavily_api_key = golish_settings::get_with_env_fallback(
        ...
        if has_tavily_api_key || config.settings.tools.web_search {
            ...
            let tavily_tools = golish_web::create_tavily_tools(tavily_state);
