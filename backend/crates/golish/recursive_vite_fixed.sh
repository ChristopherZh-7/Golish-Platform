...
while [ ${#QUEUE[@]} -gt 0 ]; do
  ...
  grep -oE 'index-[a-zA-Z0-9_-]+-[a-f0-9]{8}\.(js|mjs)' "$OUT/$FILE" 2>/dev/null | sed 's|^|/jse/|' | while read -r ref; do
  ...
  grep -oE 'import\(["\'](/jse/[^"\']+)["\']\)' "$OUT/$FILE" 2>/dev/null | sed 's|import("||;s|")||' | while read -r ref; do
    grep -qx "${ref#/jse/}" "$DONE_FILE" 2>/dev/null || QUEUE+=("$ref")
  done
  
  grep -oE 'require\(["\'](/jse/[^"\']+)["\']\)' "$OUT/$FILE" 2>/dev/null | sed 's|require("||;s|")||' | while read -r ref; do
    grep -qx "${ref#/jse/}" "$DONE_FILE" 2>/dev/null || QUEUE+=("$ref")
  done
