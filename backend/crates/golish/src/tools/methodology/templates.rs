//! Built-in methodology templates (OWASP WSTG, PTES) as hard-coded data.
//! Extracted verbatim from `methodology.rs` to keep `mod.rs` within the
//! file-size budget; behavior unchanged.

use super::types::{CheckItem, MethodologyTemplate, Phase};

pub(super) fn built_in_templates() -> Vec<MethodologyTemplate> {
    vec![
        MethodologyTemplate {
            id: "owasp-wstg".to_string(),
            name: "OWASP WSTG".to_string(),
            description: "OWASP Web Security Testing Guide - 系统化Web应用安全测试方法论"
                .to_string(),
            phases: vec![
                Phase {
