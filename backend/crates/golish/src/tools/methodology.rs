...
fn built_in_templates() -> Vec<MethodologyTemplate> {
    vec![
        MethodologyTemplate {
            ...
            phases: vec![
                Phase { id: "info-gathering".to_string(), name: "信息收集 (Information Gathering)".to_string(), description: "收集目标应用的技术架构、入口点和暴露面信息".to_string(), items: vec![
                ...
                Phase { id: "config-mgmt".to_string(), name: "配置管理测试 (Configuration)".to_string(), description: "测试应用和基础设施配置中的安全弱点".to_string(), items: vec![
                ...
                Phase { id: "identity-mgmt".to_string(), name: "身份认证测试 (Identity)".to_string(), description: "测试身份验证和会话管理安全性".to_string(), items: vec![
                ...
                Phase { id: "injection".to_string(), name: "注入测试 (Injection)".to_string(), description: "测试各类注入漏洞".to_string(), items: vec![
                ...
                Phase { id: "business-logic".to_string(), name: "业务逻辑测试 (Business Logic)".to_string(), description: "测试应用特定的业务逻辑缺陷".to_string(), items: vec![
        ...
        MethodologyTemplate {
            ...
            phases: vec![
                Phase { id: "ptes-intel".to_string(), name: "情报收集 (Intelligence Gathering)".to_string(), description: "被动和主动信息收集".to_string(), items: vec![
                ...
                Phase { id: "ptes-vuln".to_string(), name: "漏洞分析 (Vulnerability Analysis)".to_string(), description: "漏洞识别和验证".to_string(), items: vec![
                ...
                Phase { id: "ptes-exploit".to_string(), name: "漏洞利用 (Exploitation)".to_string(), description: "尝试利用已发现的漏洞".to_string(), items: vec![
                ...
                Phase { id: "ptes-post".to_string(), name: "后渗透 (Post-Exploitation)".to_string(), description: "获取初始访问后的后续操作".to_string(), items: vec![
                ...
                Phase { id: "ptes-report".to_string(), name: "报告 (Reporting)".to_string(), description: "编写测试报告".to_string(), items: vec![
...
fn check(id: &str, title: &str, desc: &str, tools: &[&str]) -> CheckItem {
    CheckItem {
        ...
        checked: false, notes: String::new(), tools: tools.iter().map(|s| s.to_string()).collect(),
    }
}
