...
//!
//! Provides Tauri commands for the frontend to write logs to ~/.golish/frontend.log
...
fn frontend_log_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".golish").join("frontend.log"))
}
...
/// Write a log message from the frontend to ~/.golish/frontend.log
#[tauri::command]
