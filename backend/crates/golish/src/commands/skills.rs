...
//! - Global: `~/.golish/skills/<skill-name>/SKILL.md`
//! - Local: `<project>/.golish/skills/<skill-name>/SKILL.md`
//!
//! This module provides thin Tauri command wrappers around the `golish-skills` crate.

use crate::error::Result;

// Re-export types from golish-skills for use by other modules
pub use golish_skills::{SkillFileInfo, SkillInfo};

/// List available skills from global (~/.golish/skills/) and local (.golish/skills/) directories.
...
pub async fn list_skills(working_directory: Option<String>) -> Result<Vec<SkillInfo>> {
    Ok(golish_skills::discover_skills(working_directory.as_deref()))
}

/// Read the full content of a SKILL.md file (frontmatter + body).
#[tauri::command]
pub async fn read_skill(path: String) -> Result<String> {
    golish_skills::load_skill_content(&path).map_err(Into::into)
}
...
pub async fn read_skill_body(path: String) -> Result<String> {
    golish_skills::load_skill_body(&path).map_err(Into::into)
}
...
pub async fn list_skill_files(skill_path: String, subdir: String) -> Result<Vec<SkillFileInfo>> {
    golish_skills::list_skill_files(&skill_path, &subdir).map_err(Into::into)
}
...
"#,
    ...
    async fn test_list_skills() {
        ...
        let skills_dir = temp_dir.path().join(".golish").join("skills");
