...
/// List available prompt files from global (~/.golish/prompts/) and local (.golish/prompts/) directories.
...
pub async fn list_prompts(working_directory: Option<String>) -> Result<Vec<PromptInfo>> {
    ...
    // Read global prompts from ~/.golish/prompts/
    if let Some(home) = dirs::home_dir() {
        let global_dir = home.join(".golish").join("prompts");
    ...
    // Read local prompts from {working_directory}/.golish/prompts/
    // Local prompts override global prompts with the same name
    if let Some(wd) = working_directory {
        let local_dir = PathBuf::from(&wd).join(".golish").join("prompts");
        if local_dir.exists() {
