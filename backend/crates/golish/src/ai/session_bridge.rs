//! App-layer implementation of `SessionPersistence` backed by `sqlx`.
...
pub struct PgSessionPersistence {
    pool: Arc<PgPool>,
}

impl PgSessionPersistence {
...
#[async_trait]
impl SessionPersistence for PgSessionPersistence {
    async fn save_session(
        ...
        sqlx::query(
            r#"INSERT INTO sessions (id, title, status, workspace_path, workspace_label, model, provider, created_at, updated_at)
               ...
               ON CONFLICT (id) DO UPDATE SET status = 'running'::session_status, updated_at = NOW()"#,
        )
        .bind(session_uuid)
        .bind(snapshot.messages.first().and_then(|m| {
            if m.role == GolishMessageRole::User {
                Some(m.content.chars().take(100).collect::<String>())
            } else { None }
        }))
    ...
    async fn finalize_session(
        ...
        self.save_session(snapshot, session_uuid).await?;
