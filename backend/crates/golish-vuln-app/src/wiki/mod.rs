...
//!
//! Public command names are unchanged. Extracted from `golish/src/tools/wiki/`
//! into the `golish-vuln-app` crate (crate-per-service split M1b); the `golish`
//! aggregate `generate_handler!` reaches these commands via the
//! `commands_facade::wiki` re-export, which globs `wiki::*` — so every command
//! must stay reachable at this module's path.
...
// `wiki::*` must be reachable at this exact path because the `golish` aggregate
// `generate_handler!` (via `commands_facade::wiki`) references each one by path.
//
...
// expands `wiki::wiki_init` into `wiki::__cmd__wiki_init!(…)`, so without the
// macro re-export the build fails with "could not find `__cmd__wiki_init` in
// `wiki`".
pub use dashboard::{
