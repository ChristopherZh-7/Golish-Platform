...
impl AgentBridge {
    ...
    ///
    /// `set_db_backend` builds the tracker with a fresh random session UUID. TaskMode
    /// and headless `--stage-run` call this with the orchestrator's durable session
    /// id so `tool_calls` are recorded under the SAME session the harness gate
    /// queries. Cloned trackers share this identity, so an immutable `Arc<AgentBridge>`
    /// can safely rebind before stage execution starts. No-op if no tracker is set.
    pub fn set_tracker_session_uuid(&self, session_uuid: uuid::Uuid) {
        if let Some(tracker) = self.services.db_tracker.as_ref() {
