...
pub fn discover_skills(working_directory: Option<&str>) -> Vec<SkillInfo> {
    ...
    // Read global skills from ~/.golish/skills/
    if let Some(home) = dirs::home_dir() {
        let global_dir = home.join(".golish").join("skills");
    ...
    // Read local skills from {working_directory}/.golish/skills/
    // Local skills override global skills with the same name
    if let Some(wd) = working_directory {
        let local_dir = PathBuf::from(wd).join(".golish").join("skills");
        if local_dir.exists() {
