# agent-progress archive · 2026-06-28

> Archived from `agent-progress.md` on 2026-06-28T10:35:53.
> Kept out of the default startup context so new agents read only recent state.
> This is an archive, not a deletion; grep here when older evidence is needed.
...
- **续（同轮）· #4 余项（PR-D 读路径扩 per-org fan-out gate）**：`org_gate.rs::evaluate_org_stage_gate` 加同款灰度 union（`technique_outcomes_read_enabled()` + org 绑定 → `repo.technique_outcome_facts(oid, session_id)` 投影 EvidenceFact，found/empty/error|blocked→Found/Empty/Error，extend facts，flag off 逐字节不变）。验证：`cargo nextest -p golish-agent-kit` → **709 passed**；`cargo clippy -p golish-agent-kit -D warnings` → exit 0；ReadLints clean；JSON VALID。**读路径现覆盖 execute.rs 主路径 + org_gate per-org fanout**。**剩余未做（均有 wrinkle，未盲接）**：① `direct/mod.rs` recon_discover_subsidiaries 写点——其 fact asset 是公司名、需 org-level 投影（project_org_level_subsidiary_facts），而该投影在 fetch_evidence_facts_for_gate 里跑在 PR-D union **之前**，故直接写+读时序对不上，需专门处理；② golish-recon-app `persistence.rs` enrich 写点（需查该处 org_id/run_id 上下文）；③ submit 预检读路径（须扩 `EvidenceLedgerQuery` seam + impl）；④ PR-E 清旧 coverage_truth union（读全切表、活体稳后）。**本会话全部成果已 commit `48d14290`**（30 文件 +1961/-68；未 push，ahead origin/main 3）：Phase1+T3+T1+T2+#4(migration+repo+写[命令路径]+读[execute+org_gate])。**仅显式暂存本会话文件**，刻意未碰他会话(agent-1)的 perdim-freshness docs（2026-06-22 ×2）/ pentest-harness-completeness-template.xlsx / gen_pentest_completeness_template.py（保持其未提交）。**未跑 full precommit**（scoped nextest+clippy 全绿替代）；push 待用户决定。剩余余项见上（recon/persistence 写点、submit 读、PR-E、评审 #5/#6）。

- **续（同轮，commit 48d14290 之后的新增未提交）· #4 余项：submit 预检读路径**：`EvidenceLedgerQuery` 加 `technique_outcome_facts`（默认空）+ `GolishDbRepoProvider` 委托 `technique_outcome_facts_impl` + `gate_context` 灰度 union（`technique_outcomes_read_enabled()` + org 绑定 + `self.session_id` 作 run_id，outcome blocked→Error）。**PR-D 读路径现覆盖 execute + org_gate + submit 全 3 条 gate 路径**。验证：`cargo nextest -p golish-agent-app` → **53 passed**；`cargo clippy -p golish-agent-app -D warnings` → exit 0；ReadLints clean；JSON VALID。剩余：recon_discover_subsidiaries 写点（company-name 投影 wrinkle）、persistence.rs enrich 写点、PR-E、评审 #5/#6。

- **续（同轮，48d14290 之后未提交）· #4 余项：recon_discover_subsidiaries 写点（provenance-only）**：`direct/mod.rs` recon `Ok(id)` 臂 mirror 命令路径 upsert（gray-switch `technique_outcomes_write_enabled` + org 绑定 + facts，非致命 warn；source=tool、query=company、evidence_ids=[id]）。**wrinkle 处理决定**：subsidiary fact 的 asset 是公司名（org 级），不进 coverage 矩阵（in-scope 资产轴不含公司名）→ 故仅记 **provenance**；subsidiary 的 coverage 仍由 db_truth 的 has_subsidiary 投影（assemble_truth_facts_typed）提供，**未改 coverage 行为**。PR-D 读到该 company-name fact 无害（不匹配任何 in-scope cell，被忽略）。真正的「technique_outcomes SUBSIDIARY 投影到 in-scope 资产」留待 PR-E 切单源时处理（届时移 PR-D union 到 project_org_level_subsidiary_facts 之前）。验证：`cargo nextest -p golish-agent-runtime` → **246 passed**；`cargo clippy -p golish-agent-runtime -D warnings` → exit 0；ReadLints clean；JSON VALID。**48d14290 之后累计未 commit**：harness_submit_tool.rs / db_bridge/evidence.rs（submit 读）+ direct/mod.rs（recon 写）+ feature_list.json + agent-progress.md。剩余 #4：persistence.rs enrich 写点（golish-recon-app，需查上下文）、PR-E；评审 #5/#6 未动。

- **续（同轮，48d14290 之后未提交）· #4 余项：persistence.rs enrich 写点（subdomain landing 代表）**：查证 golish-recon-app 有 golish-pentest-domain（canonical_asset_key ✓）但**无 golish-agent-kit**（gray-switch flag 所在）→ 错误 DAG 方向不可加依赖，故**就地读同一 env var** `GOLISH_TECHNIQUE_OUTCOMES_WRITE`（语义一致、默认 off）。`land_subdomain_assets`：循环前算 `write_technique_outcomes` flag；target_assets upsert Ok 臂内 → canonical_asset_key 归一 subdomain + `golish_db::repo::technique_outcomes::upsert`（technique=GOLISH-INTEL-SUBDOMAIN、outcome=found、source=organization_recon、query=root、evidence_ids 空、collected_at None，非致命 warn）。coverage 仍 db_truth 提供，本表此处仅 provenance。验证：`cargo nextest -p golish-recon-app` → **197 passed**；`cargo clippy -p golish-recon-app -D warnings` → exit 0；ReadLints clean；JSON VALID。**已 commit `d842eb11`**（6 文件 +134/-4；未 push，ahead origin/main 5）：harness_submit_tool.rs + db_bridge/evidence.rs（submit 读）+ direct/mod.rs（recon 写）+ persistence.rs（enrich subdomain 写）+ feature_list.json + agent-progress.md。仍仅显式暂存本会话文件（agent-1 的 perdim-freshness docs/xlsx/脚本未碰）。剩余：enrich dns/whois landing（同模式）、PR-E、评审 #5/#6。

- **续（同轮，调查后定性）· #4 代码完成**（feature_list 转 **passing**）：用户「一口气搞完 #4」→ 调查 dns/whois enrich landing 写点后定性为**范围外（设计使然）**：`land_dns_records` 仅 `refresh_per_asset_landing`（无 run_id）调用、`land_whois` 经 `recon_lookup_whois` 工具 `execute(args, _workspace)`（Tool trait 签名无 session/run_id）——`technique_outcomes.run_id` NOT NULL 需 run 隔离，**run-less 上下文不宜写**（强塞 sentinel 会破坏 run 隔离）；且 DNS/WHOIS coverage **已由 db_truth 投影 + agent 命令路径 dig/whois 的 technique_outcomes 写覆盖**（WHOIS 经 recon 工具属 org 级，db_truth has_whois 已投影）。**PR-E**（清旧 coverage_truth union）= **live-gated**（设计明确『活体 dual-read parity 稳后才切单源』，非 code-only）。故 #4 端到端**代码完成**：表 + repo + 写[命令路径/recon-subsidiary/per-run enrich subdomain] + 读[execute+org_gate+submit 全 3 路]，双 gray-switch 默认 off。剩 PR-E（活体）+ 评审 #5/#6（另立设计）。本定性轮**无新代码**，仅改 feature_list（passing）+ progress，随 bookkeeping commit。
...
- **关键约束（多 agent 共树）**：branch `feat/stage-run-fanout` 工作树混有多 agent 未提交改动（org_stage_completions 迁移/repo、ai/tracking_bridge、AIChatPanel、tools.ts、stage_run_call.rs 等）。本次**只 stage Phase 0/1/2a 相关文件**，其它会话半成品原样保留未暂存。
- **精准暂存（含 hunk 级拆分，未用 git add -A）**：
  - 整文件 stage：Phase 0（`golish-db/.../targets.rs`、`organization_recon/{persistence,commands,mod}.rs`、`golish/src/commands_registry.rs`、`lib/api/organization-recon.ts`、模块卡 `organization_recon.md`）；Phase 1（`target-panel/org-tree.ts`、`OrgTreeSidebar.tsx`、`TargetGroupedView.tsx`、新 `host-tree.test.ts`）；Phase 2a（`technique_resolver.rs`、`stage_spec.rs`、`gate/rule_engine.rs`、`gate/finding_verification_check.rs`）；设计/计划 4 doc；`feature_list.json`、`agent-progress.md`。
  - **hunk 级部分暂存**（`git apply --cached --recount`，排除他会话 hunk）：`target_intel.json` 仅 `host_aware_coverage`（排除他人 `findings_allowed`/recon-stage-findings-suppression）；`en/zh-CN.json` 仅 host-view 标签（viewByType/viewByIp/unresolvedGroup/assets*），**排除** MCP-1 的 `wechat_official_accounts`（MCP-1 已注明该 i18n 标签待协调单独提交）。
  - **排除（他会话未提交，保留未暂存）**：`repo/mod.rs`(org_stage_completions 注册)、`org_stage_completions.{rs,sql}`、`harness_submit_tool.rs`、`ai/tracking_bridge`、`db_tracking`、`db_traits/tracking.rs`、`stage_run_call.rs`、`AIChatPanel`/`ApprovalModeSelector`/`ToolCallSummary`/`ReconIntelSummaryLine`/`SubAgentDetailView`/`ToolCallDetailView`、`useAppLifecycle`/`ansi`/`conversation-db-sync`/`tools.ts`/`actions.ts`、`JsonView/`、`enumeration/external_attack_surface/scoping.json`、`amass.json`。
- **验证依据（本会话未重跑·有据）**：依赖 MCP-2 本日已跑绿的范围测——终端 285181 `nextest -p golish-agent-kit technique_resolver host_aware coverage_complete stage_spec` → EXIT=0；514037 `… all_twelve_stage_specs host_aware target_intel` → EXIT=0；progress 另记 golish-db targets 12/12、golish-recon-app organization_recon 52/52、check-fe exit 0、host-tree vitest 2/2 + test-fe 1324。**本会话未重跑全量 `just precommit`**：`backend/target` 192G、磁盘仅余 ~2.8GiB + 多 agent 并发抢 cargo build lock，重跑风险高且会污染其它会话；按用户「commit」指示先落库，全量 precommit 留待磁盘清理后单独跑。提交子集仅引用已 stage 文件（无对排除文件的编译依赖），standalone 可编译。
- **未做 / 边界**：未 push（§2.7 待用户单独点头）；未跑全量 precommit；活体未验；其它会话半成品仍未暂存。
- **已参考技能**：无（本轮为 git 精准暂存/提交编排，按 verification-before-completion 精神逐文件核对暂存集；未 Read 本地 skill raw；`mcp-server/role-skills/` 为空 Glob 0）。

---

### 2026-06-15 · Host-aware coverage Phase 2a 实现（按计划 TDD，验证绿）（BaJie MCP-agent-2 · DISPATCH off · 同会话续 · 用户「实现 2a」）

- **本轮目标**：按 `docs/superpowers/plans/2026-06-15-host-aware-coverage.md` 实现 Phase 2a（target_intel coverage 按资产类型分技术），executing-plans + TDD。
- **改动（5 文件，未 commit）**：
  - `golish-agent-kit/src/harness/technique_resolver.rs`：`AssetClass::from_value(&str)`（按值判类型：IpAddr→Ip、http(s)→Url、addr/prefix→Cidr、空→Other、否则 Domain）+ `technique_applies(stage,class,tech)`（target_intel 矩阵：SUBDOMAIN 仅 Domain；DNS/CT 仅 Domain|Url；WHOIS/ASN/OSINT 全；Other→全 fail-safe；非 target_intel stage 全 true=2a no-op）+ `techniques_for` 便捷 + 4 测。
  - `golish-agent-kit/src/harness/stage_spec.rs`：`host_aware_coverage: bool`（`#[serde(default)]`，仿 facts_from_db_truth）+ 解析测。
  - `golish-agent-kit/src/harness/gate/finding_verification_check.rs`：全 StageSpec 字面量补 `host_aware_coverage: false`。
  - `golish-agent-kit/src/harness/gate/rule_engine.rs`：`coverage_complete` 内层循环——`spec.host_aware_coverage` 开时按 `AssetClass::from_value(asset)` + `technique_applies(spec.kind,…)` 过滤 `techniques`（关时 `techniques.iter().collect()` 逐字节不变）+ parity 测 `host_aware_coverage_relaxes_ip_not_domain`（域名 6 项不变 / IP 掉 SUBDOMAIN/DNS/CT）。
  - `resources/harness/stages/target_intel.json`：`host_aware_coverage: true` + comment（**最后一步、parity 绿后才开**）。
- **关键实现决策（优于设计 §4.1）**：发现 `coverage_complete(d, spec, ctx, …)` 本就收 `&StageSpec`（含 `spec.kind`）+ `ctx.in_scope_assets`，故 2a **零 GateContext/hook/DB 改动**——全部落在 gate + technique_resolver + stage_spec（设计已加 §4.0 记录此精简）。
- **TDD（先红后绿）**：先写 technique_resolver/stage_spec/rule_engine 测（红：函数/字段未定义），再实现转绿。
- **运行过的验证（实跑·有据）**：
  - `cargo nextest -p golish-agent-kit technique_resolver host_aware coverage_complete stage_spec` → **48/48 passed**（含全部新测 + parity；既有 coverage_complete/stage_spec 无回归）。
  - 启用 JSON flag 后 `cargo nextest -p golish-agent-kit all_twelve_stage_specs host_aware target_intel` → **13/13 passed**（`all_twelve_stage_specs_load_and_kind_matches` 确认 target_intel.json 带新字段正常加载）。
  - `python3 json.load target_intel.json` → host_aware_coverage=True；grep 全 backend `findings_allowed:` 仅 2 处（struct def + 已修的测字面量）→ 无其它 crate 构造全字面量，golish 编译链不破。
- **环境注意（给后续 agent）**：本机 cargo 跑被**并发**（另一 agent 同时跑 golish-agent-kit nextest，抢 target build lock）+ 环境多次 kill 长命令打断；最终用**后台 + 重定向 /tmp 日志**（`cargo nextest … > /tmp/x.log 2>&1`）跑通。前台长 cargo 在本环境约 40s 被 abort。
- **未做 / 边界**：未跑全量 `just precommit`（test-rust-all 太重 + 环境打断）；未 commit、未 push（用户说「实现」非「commit」）；活体未验（--stage-run 混合 域名+IP scope 看 IP 不再无限 BLOCK）；**Phase 2b（EAS+enum 矩阵）+ 2c（authoritative type 轴 + IP 原生技术）未做**。
- **状态**：`feature_list.json` host-aware → `in_progress`（2a 实现+测绿；2b/2c + 全量 precommit 待）。
- **下一步建议**：① 用户定 commit（Phase 0/1 + 2a 一并或分批）；② 跑全量 precommit（环境空闲时）；③ 活体复看；④ 之后做 2b。
- **已参考技能**：无（按 TDD 先红后绿、executing-plans 逐任务、verification-before-completion 跑命令拿证据[48+13 测绿]的**精神**执行；本轮**未 Read** 这些本地 skill 的 raw，故不标 must·已 Read；本会话此前已 Read `.cursor/skills/writing-plans`）；`mcp-server/role-skills/` 为空（Glob 0）。

---

### 2026-06-15 · Host-aware coverage Phase 2a 实现计划（BaJie MCP-agent-2 · DISPATCH off · 同会话续 · 用户「按默认写实现计划」）

- **本轮目标**：用户按设计 §8 4 个默认拍板（2a only / drop-only / URL 继承域名 / classify-from-value），写实现计划（**零生产代码**，仅文档）。用 writing-plans 技能（本轮**已 Read** `.cursor/skills/writing-plans/SKILL.md`）。
- **改前读实代码（关键发现，简化了方案）**：`coverage_complete(d, spec, ctx, …)` **本来就收 `&StageSpec`（含 `spec.kind`）+ `ctx.in_scope_assets`（值）**，所以 2a 可**完全在 gate 内实现**——不用动 GateContext/hook/DB（设计 §4.0 已据此加「精简实现」节）。另确认 `StageSpec.facts_from_db_truth: bool` 是 `#[serde(default)] bool` 灰度 flag 的现成先例；`technique_resolver.rs::stage_baseline` 确认 target_intel=6 被动情报。
- **产出（1 新文件 + 3 更新，零代码）**：
  - `docs/superpowers/plans/2026-06-15-host-aware-coverage.md`：Phase 2a 四任务带**完整代码块** + TDD（先红后绿）+ 验证命令 + 每任务 commit：A.1 `technique_resolver`（`AssetClass::from_value` 按值分类 + `technique_applies(stage,class,tech)` 矩阵 + `techniques_for` + 测）；A.2 `StageSpec.host_aware_coverage` flag（仿 facts_from_db_truth，默认关，+解析测，+修全字面量）；A.3 `coverage_complete` 内层循环按资产类型过滤 `techniques`（flag 开才过滤，关=逐字节不变）+ PASS/BLOCK parity 测（域名 6 项不变、IP 掉 SUBDOMAIN/DNS/CT）；A.4 target_intel.json 开 flag（最后）+ 全量验证。2b/2c sketch。
  - 更新：设计 §4.0「精简实现」节 + 状态行（计划已出）；`feature_list.json` host-aware 条目 plan 字段填实路径。
- **运行过的验证**：`python3 json.load feature_list.json` → OK（本轮零代码，无测可跑）。
- **未做 / 边界**：实现未开始（计划态）；零代码；未 commit。Phase 0+1 仍待 commit。
- **下一步建议**：① 用户审计划（尤其 A.3 parity 测口径）→ 同意后按 executing-plans 逐任务实现 2a；② 或先 commit 已就绪的 Phase 0+1。
- **已参考技能**：`.cursor/skills/writing-plans`（must·**已 Read**，按其头部/任务结构/禁占位符/自检写计划）；`.cursor/skills/test-driven-development`（conditional·计划内每任务先红后绿，未重读 raw）；`mcp-server/role-skills/` 为空（Glob 0）。

---

### 2026-06-15 · Host-aware coverage（IP-centric Phase 2 / 方案 C）设计文档（BaJie MCP-agent-2 · DISPATCH off · 同会话续 · 用户「去写 Phase 2 设计」）

- **本轮目标**：先经多轮答疑澄清 Phase 2 范围（用户关键纠偏：coverage 是**按阶段**算的，URL 目录/参数、端口扫描属后续阶段，target_intel 是**被动情报**；Phase 2 = 把每类资产的「该交什么作业才算合格」分开，不是改证据存储），再写设计文档（**零代码**）。
- **改前读实代码确认现状（未读不引）**：
  - `GateContext`（`harness/gate/rule_engine.rs:194`）= `{ in_scope_assets: Option<Vec<String>>(仅 value), expected_techniques: Option<Vec<String>>(一份), evidence_facts }`。
  - `coverage_complete`（rule_engine.rs:385）= `for asset in assets { for tech in techniques }` 双循环，**一份 techniques 套每个资产**（type-blind）。
  - 期望技术链：`gate_expected_techniques(execute.rs:1714)` → `DefaultSprintContractGenerator::expected_techniques_for(sprint_contract.rs:121)` → `resolve_expected_techniques(technique_resolver.rs:66)`，取 scope 资产类型**并集**出一份（唯一裁剪：全 scope 无 web 才 drop PARAM）。
  - `assemble_truth_facts`（coverage_truth.rs:176）org 级技术（ASN/CT/WHOIS/OSINT）**无差别盖每资产**；per-asset（SUBDOMAIN/DNS…）按值集命中。
  - 实证后果：in-scope **IP** 在 target_intel 永远缺 SUBDOMAIN + 正向 DNS 两格（IP 无子域名/无正向 A）→ 永远「没做完」→ 无限 BLOCK 或被瞎填；CT/ASN/WHOIS/OSINT 是 org 级凭空盖上。
- **产出（1 新文件 + 2 cross-ref，零代码）**：
  - `docs/design/2026-06-15-host-aware-coverage.md`：§1 现状证据；§3 技术×资产类型矩阵（target_intel：域名核 6、IP 只核 ASN/WHOIS/OSINT、drop 域名专属项；EAS=主机级；enum=URL级）；§4 seam 改法（GateContext 带 asset 类型 + coverage_complete 按类型选技术 + technique_resolver 加 `techniques_for(stage,AssetClass)` 矩阵；fail-safe：未知类型保留全清单）；§5 分期 2a(仅 target_intel,不加采集器,低风险)/2b(EAS+enum)/2c(full,改 truth 投影 + IP 原生技术)；§6 灰度 flag + before/after PASS/BLOCK parity 测（唯一允许 delta = IP/CIDR 的 SUBDOMAIN/DNS/CT BLOCK→PASS，域名零变化）+ 用户点头；§7 touch points；§8 4 开放项；§9 风险/非目标。
  - cross-ref：`docs/superpowers/plans/2026-06-15-ip-centric-asset-model.md` Phase 2 段加指针；`feature_list.json` 新增 `host-aware-coverage-2026-06-15` = `not_started`。
- **运行过的验证**：`python3 json.load feature_list.json` → OK（本轮零代码，无测试可跑；设计阶段）。
- **未做 / 边界**：未写实现计划（等用户审 §3 矩阵 + §8 开放项）；零代码改动；未 commit、未 push。
- **下一步建议**：① 用户审设计（尤其 §3 矩阵对不对、§8 开放项默认 2a-only/drop-only 是否同意）；② 同意后写 `docs/superpowers/plans/2026-06-xx-host-aware-coverage.md`（TDD）；③ 之前的 Phase 0+1 仍待 commit。
- **已参考技能**：无（本轮按 brainstorming/writing-plans 的**精神**——先与用户澄清范围、再结构化设计——执行，但**未 Read** 这两个本地 skill 的 raw，故不标 must·已 Read；写的是 design 非 plan；`mcp-server/role-skills/` 为空 Glob 0）。

---

### 2026-06-15 · IP-centric 资产树 Phase 1（前端 IP 归组树 + 按类型/按 IP 切换）（BaJie MCP-agent-2 · DISPATCH off · 接 Phase 0 同会话续 · 用户「接着做 Phase 1 前端」）

- **本轮目标**：在 Phase 0（后端 `real_ip` 落点）之上做前端 IP 归组树（方案 B Phase 1，按 `docs/superpowers/plans/2026-06-15-ip-centric-asset-model.md` Task 1.1-1.4，TDD）。
- **关键改前定位修正**：计划 snippet 用 `t.target_type`，但本仓库前端 `Target`（`@/lib/pentest/types` = ts-rs `BackendTarget` & {ports}）字段是 **`type`**（`TargetType = "domain"|"ip"|"cidr"|"url"|"wildcard"`，IP 值是 `"ip"`），故实现统一用 `target.type`、`target.real_ip`。
- **改动（4 文件 + 1 测试，未 commit）**：
  - `frontend/lib/target-panel/org-tree.ts`：`OrgTreeNode` 加 `kind?: "org"|"host"|"bucket"`；新增 `buildHostTree(orgs, targets, unassignedLabel, unresolvedLabel)`——复用 `buildOrgTree` 的 org 骨架，再把每个节点的平铺 targets 按 IP 归组（IP target 自成 host 节点 keyed by value；域名/url 挂到其 `real_ip` 的 host；无 `real_ip` 进 per-org『未解析』bucket，I8 不隐藏）；新增 `UNRESOLVED_KEY`。
  - `frontend/lib/target-panel/host-tree.test.ts`（新）：2 例（域名挂到 `real_ip` 主机节点 + 未解析进 bucket + org.targets 清空；org spine 保持）。
  - `frontend/components/TargetPanel/OrgTreeSidebar.tsx`：`OrgTreeNodeRow` 加 `kind/isOrg/isHost`；host 用 `Network` 图标、bucket/unassigned 用 `FolderOpen`、org 用 `Building2`；host/bucket 行的点击只 toggle 折叠（不 setSelectedOrgId）、且**不渲染** org 操作区（`{isOrg && (...操作区...)}`，biome 自动重排缩进）；资产子组 + 递归子节点渲染原样复用。
  - `frontend/components/TargetPanel/TargetGroupedView.tsx`：加 `viewMode: "byType"|"byIp"`（默认 byType）+ `roots` memo 按 viewMode 选 `buildHostTree`/`buildOrgTree`；工具栏加分段切换按钮（`translateWithFallback` 标签）；import `buildHostTree` + `cn`。
  - `frontend/lib/i18n/{en,zh-CN}.json`：`targets.viewByType`/`viewByIp`/`unresolvedGroup`（By type/By IP/Unresolved · 按类型/按 IP/未解析域名）。
- **TDD（先红后绿）**：先写 `host-tree.test.ts` → `pnpm test:run host-tree` 看红（`buildHostTree is not a function`）→ 实现 → 转绿（2/2）。
- **运行过的验证（实跑·有据）**：`pnpm test:run host-tree` → **2/2 绿**；`just test-fe`（全量 vitest）→ **123 files / 1324 passed / 12 skipped 无回归**；`just check-fe`（biome + tsc）→ 首次因 `{isOrg &&` 包裹后缩进报 format → `pnpm exec biome check --write`（Fixed 1 file）→ 复跑 **exit 0**；ReadLints 5 文件无错。
- **已记录证据**：见上命令 + 计数 + 退出码。
- **未做 / 边界**：未跑全量 `just precommit`（test-rust-all 太重；前端改动已 test-fe/check-fe 全覆盖，后端本轮零改）；未 commit、未 push（§2.7 等用户点头）；活体未验（回填 + 切『按 IP』看树）；**Phase 2（host-aware coverage，动 harness 核心）deferred 待单独立项 + 用户点头**。
- **状态**：`feature_list.json` 的 `ip-centric-asset-model-2026-06-15` Phase 1 标 ✓（整条仍 `in_progress`，待全量 precommit + commit；Phase 2 deferred）。
- **下一步建议**：① 用户定 commit / push（Phase 0+1 共 9 改 + 1 新测 + 设计/计划 doc + 模块卡 + progress/feature_list）；② 可选全量 precommit；③ 活体回填复看；④ 之后再单独立 Phase 2。
- **已参考技能**：`.cursor/skills/executing-plans`（must·已 Read）、`.cursor/skills/test-driven-development`（must·已 Read，host-tree 红→绿）、`.cursor/skills/verification-before-completion`（conditional·跑 test-fe/check-fe 拿证据再宣称）；`mcp-server/role-skills/` 为空（Glob 命中 0）。

---

### 2026-06-15 · IP-centric 资产树 Phase 0（后端 real_ip 落点 + 回填命令）（BaJie MCP-agent-2 · DISPATCH off · 接 MCP-1 上下文转移 · 用户「开 Phase 0 回填」）

- **本轮目标**：接 MCP-1 上下文（IP-centric 资产树设计 + 计划已写，方案 B / C 列 Phase 2 已拍板）。先回答 MCP-1 遗留的概念问题（「现在 coverage 对 intel 的逻辑本来就不对吗」——读真实代码确认：`technique_resolver.rs` 按 stage+scope 资产类型并集出**一份**期望技术、非 per-asset，`coverage_complete` 对每个资产同列核，`assemble_truth_facts` 又把 org 级技术无差别盖每资产 → per-asset×per-type 精确矩阵不存在 = Option C/Phase 2 要补），再按计划开 **Phase 0**。
- **Phase 0 三 task 全做完（按 docs/superpowers/plans/2026-06-15-ip-centric-asset-model.md，TDD）**：
  - **Task 0.1** `golish-db/src/repo/targets.rs`：加 `set_real_ip_by_id`（无 scope，调用方持 id，幂等覆盖）+ `build_backfill_real_ip_sql`（`DISTINCT ON(target_id) ... record_type='A' ORDER BY created_at` 取最早 A，仅 `real_ip=''` 才填，`($1 IS NULL OR project_path=$1)`）+ `backfill_real_ip_from_dns`（返 rows_affected）+ SQL-shape 单测（先写测后实现）。
  - **Task 0.2** `golish-recon-app/src/organization_recon/persistence.rs::land_dns_records`：解析落 `dns_records` 后顺手把首个 A（否则首条）写进 `targets.real_ip`。**实现偏差（优于计划字面）**：计划 snippet 在 `for ... in records`（消费 move）之后再借 `records` 会 borrow-after-move；改为**消费循环前先捕获** `primary_ip: Option<(Uuid,String)>` 再 `set_real_ip_by_id`。
  - **Task 0.3** 命令 `recon_backfill_real_ip`（`commands.rs`，取 `DbState` → `pool_ready` → `backfill_real_ip_from_dns`，`<domain>_<verb>_<object>` I4）；`mod.rs` 加 `#[doc(hidden)] pub use commands::__cmd__recon_backfill_real_ip`；`golish/src/commands_registry.rs` generate_handler! 列 ident（facade `organization_recon.rs` 本就 glob re-export，未动 facade）；前端 `lib/api/organization-recon.ts` 加 `backfillRealIp(projectPath?)` wrapper（M2 不裸 invoke，`projectPath ?? null`）。
- **前置事实核实（改前定位）**：`real_ip` 列 `TEXT NOT NULL DEFAULT ''`（migration 20260413000001，故 `real_ip=''` 谓词能覆盖全部未填行，无 NULL）；`dns_records.created_at` 存在（20260612000001）→ 回填 SQL `ORDER BY created_at` 有效；`record_type` 是列名（非 `type`）。
- **运行过的验证（实跑·有据）**：
  - `cargo nextest -p golish-db targets` → **12/12 passed**（含新 `backfill_real_ip_sql_picks_first_a_record_for_unset_targets`）。
  - `cargo nextest -p golish-recon-app organization_recon` → **52/52 passed**（land_dns_records 改动无回归）。
  - `cargo check -p golish` → **exit 0**（generate_handler! 解析 `recon_backfill_real_ip`，注册链通）。
  - `cargo clippy -p golish-db -p golish-recon-app --all-targets` → **exit 0 零告警**。
  - `just check-fe`（biome + tsc）→ **exit 0**（wrapper 类型通）。
  - `python3 -c json.load feature_list.json` → OK；ReadLints 5 文件无错。
- **已记录证据**：见上各命令 + 退出码 + 计数。
- **未做 / 边界**：未跑全量 `just precommit`（`test-rust-all` 太重；改动隔离在 golish-db repo + recon 落点 + 注册 + 前端 wrapper，已分层单测/check/clippy 覆盖）；未 commit、未 push（AGENTS.md §2.7，待用户点头）；活体未验（对 `Test1` 调 `recon_backfill_real_ip` 看 5 个有 A 记录的域名 `real_ip` 填上）；**Phase 1（前端 buildHostTree + 按类型/按 IP 切换 + i18n）未做；Phase 2（host-aware coverage，动 harness 核心）deferred 待用户单独点头**。
- **状态**：`feature_list.json` 新增 `ip-centric-asset-model-2026-06-15` = `in_progress`（Phase 0 已绿，Phase 1/2 未做）。模块卡 `docs/modules/backend/golish-recon-app/organization_recon.md` 已更新（命令 + land_dns_records real_ip 落点）。
- **下一步建议**：① 用户定 commit / push（本轮 5 文件 + 设计/计划 doc + progress + feature_list + 模块卡）；② 接 Phase 1 前端 IP 树；③ 可选活体回填复看。
- **已参考技能**：`.cursor/skills/executing-plans`（must·已 Read，按计划逐 task + 频繁验证）、`.cursor/skills/test-driven-development`（must·已 Read，Task 0.1 SQL-shape 测先行）；`.cursor/skills/verification-before-completion`（conditional·按精神跑命令拿证据再宣称）；`mcp-server/role-skills/` 为空（Glob 命中 0），候选为空。

---

### 2026-06-15 · 0.zone apk 微信公众号映射补全（BaJie MCP-agent-1 · DISPATCH off · 接 MCP-3 上下文转移 · 用户「补 apk 公众号映射」）

- **本轮目标**：接 MCP-3 上下文（http.rs 双 bug 已修 + 真机验过 0.zone）的遗留待办——0.zone `query_type=apk` 里「微信公众号」型条目（`msg.wechat_id` 有、`msg.app_url` 无）被现有 `mobile_apps` 规则（门控 `msg.app_url`）整条丢弃，需补映射。
- **根因 / 数据形状**：`golish-intel-providers/src/zone/types.rs::ApkMsg` 公众号条目带 `wechat_id`/`introduction`、无 `app_url`；asset_intel 走 **JSON 驱动 `0-zone.json` normalize**（非 typed zone provider 路径，后者 `map_apk` 早已抽 `wechat_id`，故只需补 JSON 侧）。
- **改动（4 实现文件 + 2 测试文件，未 commit）**：
  - `resources/intel-providers/0-zone.json`：新增 profile_field 规则 `source_field:[title, msg.wechat_id] → wechat_official_accounts`（`target_kind=intel`），`when msg.wechat_id exists`——只收公众号、不污染普通 site/domain（无 wechat_id）。
  - `backend/crates/golish-recon-app/src/asset_intel/profile_patch.rs`：`is_intel_array_profile_field` 加 `wechat_official_accounts`（多公众号去重累积成数组，而非标量首值覆盖）。
  - `frontend/lib/target-panel/org-fields.ts`：`INTEL_FIELD_LABELS` / `INTEL_RECORD_LABELS` / `APP_INTEL_KEYS` 加 `wechat_official_accounts`（落「Apps & Mini Programs」组）。
  - `frontend/lib/i18n/{en,zh-CN}.json`：`fields.wechat_official_accounts` = "WeChat official accounts" / "微信公众号"。
  - 测试：`backend/.../asset_intel/tests.rs`（+2）、`frontend/components/TargetPanel/TargetGroupedView.actions.test.ts`（apps 组断言）。
- **设计决策（自主拍板，可调）**：用**独立字段** `wechat_official_accounts` 而非塞 `mobile_apps`（公众号 ≠ App；既有测试刻意禁止 mobile_apps 回退 `title` 防 SEO 标题污染）；值优先公众号名 `title`、缺失回退 `wechat_id`。
- **TDD（先红后绿）**：先写 3 个失败测试（normalize 行为：公众号→`wechat_official_accounts` 且 site 不污染；profile_patch 数组折叠；前端 apps 组含新字段）→ 看红（空 entries / 标量非数组 / apps 组缺字段，原因正确）→ 实现 → 转绿。新测：`zone_apk_wechat_official_account_maps_to_profile_field` + `build_profile_patch_collects_wechat_official_accounts_as_array`。
- **运行过的验证（实跑·有据）**：`cargo nextest -p golish-recon-app asset_intel` → **66/66**（含新测 + 既有 mobile_apps / 部分成功落库测，无回归）；`cargo clippy -p golish-recon-app --all-targets -- -D warnings` → **exit 0 零告警**；前端全量 vitest → **1322 passed / 12 skipped**；`just check-fe`（biome+tsc）→ **exit 0**；ReadLints 4 文件无错。
- **commit 记录**：已 commit **`3ea3466d`**（8 文件，+807/-355，branch `feat/stage-run-fanout`，**未 push**）。因 `0-zone.json`/`asset_intel/tests.rs` 与 MCP-3 的 0.zone http 修复（partial-success/retry/timeout，含 `http.rs`+`models/asset_intel.rs`+`models/tests.rs`）共文件且互为编译/测试依赖，故按「连贯的 0.zone asset-intel 单元」一并提交（MCP-3 那批此前用户已验过真机）。**排除**：`i18n/{en,zh-CN}.json`（与他会话「assets」hunk 共文件，未纳入；feature 经 org-fields.ts 英文回退仍显示，中文标签待协调单独提交）、其余无关前端改动、`amass.json`、`agent-progress.md`（本条）。
- **未做 / 边界**：未跑全量 `just precommit`（`test-rust-all` 太重；改动隔离在 recon-app 配置 + 前端展示，已充分单测）；未 push（AGENTS.md §2.7）；i18n 标签未提交（见上）；真机 = 对某 org 点「补字段」看公众号填进「Apps & Mini Programs」。
- **下一步建议**：用户定 commit / push + 可选全量 precommit + 真机复看；如要也把公众号 `wechat_id` 单列可二期加规则。
- **已参考技能**：`.cursor/skills/test-driven-development`（must·已 Read，严格红-绿循环）；`.cursor/skills/brainstorming`（conditional·按精神先探数据形状/字段归属，未读 raw）。
...
- **A. stage_run 逐公司钻取（前端为主 + 修一处编译漏改）**：
  - **关键发现**：后端早已实现（`stage_run_call.rs` 每 org 独立 `org_request_id = {tool_id}::org::{org_id}`，派子 agent + 写进事件 `agent_request_id`；`HarnessTraceKind::StageRunOrgProgress.agent_request_id` 字段 + 单测已在）。但 **CLI `fleet.rs` 的 emit 漏改了 `agent_request_id`**（上一轮半成品）→ 工作树本来编译不过；已补 `agent_request_id: None`（CLI 跑 orchestrate 不派可追踪子 agent）。
  - 前端：`harness-handlers.ts` 读 `agent_request_id`→`row.agentRequestId`；`StageRunOrgRows.tsx` 行加 `agentRequestId` + 有子 agent 的行可点击（hover + ▸）；`ToolCallDetailView.tsx` 点行→压栈该 id + 切 `sub-agent-detail`（复用现成 SubAgentDetailView）；`SubAgentDetailView.tsx` 返回时若上一层非子 agent（= stage_run 工具行）→ 回 `tool-detail`（公司列表）而非时间线。`harness-handlers.test.ts` 补 agentRequestId 断言。ts-rs 重生成（GeneratedAiEvent/HarnessTraceKind 加 agent_request_id）。
- **B. 聊天卡 stage_run 进度**：`ToolCallSummary.tsx` 的 `ToolCallCard`：tc.name==="stage_run" 时按 requestId 取 `session.stageRun.summary`，渲染「X/Y 覆盖 · N 进行/排队/阻塞」+ 细进度条。纯展示、复用已有 summary、零后端改动。
- **C. 目标状态按阶段细分（深改，跨 DB+Rust+ts-rs+前端+工具）**：设计 `docs/design/2026-06-14-target-status-stage-aligned.md`。`target_status` 旧 `new/recon/recon_done/scanning/tested` → 新 `new/passive/active/enumerated/vuln_scan/verified`（对齐 StageKind + recon PassiveInternet/ActiveCollection）。
  - **DB**：新 migration `20260614000001_target_status_stage_aligned.sql`（PG 枚举 text 往返：drop default→列转 text→值映射 recon→passive/recon_done→active/scanning→vuln_scan/tested→verified→drop+recreate type→列转回→恢复 default；全 DDL 事务；仅 targets.status 用该类型故安全；forward-only I10）。
  - **Rust**：`golish-app-core domain::targets::TargetStatus`（变体 + serde 由 lowercase 改 **snake_case** 使 wire==DB==as_str；as_str + from_str 含 legacy 别名）；消费方仅 from_str/as_str 无穷举 match。新增单测 4 个（新值/legacy/serde 一致/as_str）。
  - **工具/AI**：`manage_targets` status 枚举改新值 + 描述改新生命周期 + 内嵌 skip 指引（list 看 status，`status≥本阶段`则跳过；update_status 逐阶段前推）。
  - **前端**：regen `TargetStatus.ts`；`TargetTreeRow.STATUS_CONFIG`（穷举,typecheck 强制）新徽章/颜色；`buildTopologyModel` 状态字面量。（TargetTimeline 的 entry.status 是审计事件态,非 TargetStatus,未动。）
- **跑过的验证（实跑全绿）**：`just check` → fmt ✓ / check-fe ✓ / test-fe ✓（前端全量） / **lint-rust（clippy --workspace -D warnings）✓ 零告警** / **test-rust-all（nextest --workspace）✓ 全过**；唯 **check-types ✗**——**仅因重生成的（正确的）`frontend/lib/generated/` bindings 未 commit**（`git diff --exit-code` 非零），与 Rust 源一致、提交后即 exit 0（同历轮惯例）。另：`nextest -p golish-app-core -p golish-recon-app -p golish-pentest-app` 256/0、`clippy` 三 crate -D 0 告警、`cargo fmt --check` clean、ReadLints 全清。
- **commit 记录**：未 commit（无显式授权；「全部搞完」按惯例理解为完成实现而非提交）。
- **未提交的文件（本轮）**：`?? docs/design/2026-06-14-target-status-stage-aligned.md`、`?? backend/crates/golish-db/migrations/20260614000001_target_status_stage_aligned.sql`、`M backend/crates/golish-app-core/src/domain/targets.rs`、`M backend/crates/golish-pentest-app/src/pentest_bridge/manage_targets.rs`、`M backend/crates/golish/src/stage_run/fleet.rs`、`M backend/crates/golish-agent-runtime/.../direct/stage_run_call.rs`（仅 fmt）、`M frontend/services/ai-events/harness-handlers{,.test}.ts`、`M frontend/components/Engagement/StageRunOrgRows.tsx`、`M frontend/components/ToolCallDetailView/ToolCallDetailView.tsx`、`M frontend/components/SubAgentDetailView/SubAgentDetailView.tsx`、`M frontend/components/AIChatPanel/ToolCallSummary.tsx`、`M frontend/components/TargetPanel/TargetTreeRow.tsx`、`M frontend/components/TargetPanel/topology/buildTopologyModel.ts`、`M frontend/lib/generated/{TargetStatus,GeneratedAiEvent,GeneratedHarnessTraceKind}.ts`。
- **风险 / follow-up**：① migration 活体仅在 app 启动时跑（用户 embedded PG）——标准事务化 DDL，已尽量稳；② 目标状态「谁翻」一期=AI 经 manage_targets 翻+读跳过，二期可让 recon/scan 流程自动翻（未做）；③ 活体 GUI 未跑（钻取/进度/徽章需真 LLM run，在用户环境）；④ **check-types 需 commit bindings 才 exit 0**。
- **下一步**：用户决定是否 commit（一次性把本轮 + 之前 stage_run drill-in 一起提交；提交后 `just check` 全绿）+ 是否活体复看。

---

### 2026-06-14 · 移除 engagement 总览/扇出全栈 + stage-run 闭环（BaJie MCP-agent-2 · DISPATCH off · 用户逐步授权 S1→S4）

- **本轮目标**：用户从「为什么新对话也冒 engagement 总览卡」追到「这功能跟 stage-run 冗余、不需要」，最终授权**全栈删除 engagement 总览/扇出 + 工人池 + worker-scope**，并要求 **stage-run 逻辑闭环**（删完 stage-run 不受影响）。
- **关键发现（删前取证）**：① GUI 那张 stage-run 单卡由 `golish-agent-runtime/.../tool_execution/direct/stage_run_call.rs` emit `StageRunOrgProgress` 驱动，**不依赖 engagement**（与 `engagement::fleet_run::OrgFleetExecutor` 是两个独立的事件生产者）→ 删 engagement 不影响 GUI stage-run。② 唯一耦合点：CLI `--stage-run` 子公司扇出借用了 `engagement::scheduler`/`OrgFleetExecutor`；「闭环」=把这个调度内核搬进 `stage_run` 自己家。
- **S1 闭环（已验证）**：`git mv engagement/scheduler.rs → stage_run/scheduler.rs`；新建 `stage_run/fleet.rs`（OrgFleetExecutor/AlwaysRunOracle/NoopScorer/CliFleetProgress + title_case）；scheduler 维持 `pub`（通用已测内核，避免 pub(crate) 触发 dead-code）；`stage_run/` 零 `use crate::engagement`（grep 无匹配）。验证：`cargo check`+`clippy -p golish` exit 0 + `nextest -p golish scheduler fleet` 13/0。
- **S2 后端拆除（已验证）**：删整个 `golish/src/engagement/`（mod/scheduler/weakness/contract/query/run_command/fleet_run）+ 5 Tauri 命令（engagement_get_snapshot / engagement_run_fleet / engagement_set·get·clear_worker_scope）+ commands_facade/engagement.rs + registry 条目 + bridge 的 `EngagementWorkerScope`（类型/字段/方法/构造/re-export）+ `core/chat.rs` worker-scope 应用 + 5 个 ts-rs DTO。**保留** `golish-db engagement_truth` repo/DB（coverage gate 用）。验证：`clippy -p golish` exit 0 零告警 + `nextest -p golish-agent-bridge -p golish-agent-app -p golish` 284/0。
- **S3 前端拆除（已验证）**：删 16 文件（lib/engagement/{pool,spawn,runPool,rolePersistence,pool.test}、components/Engagement/{EngagementOverview,EngagementInlineCard,engagementOverview.utils,.test}、lib/api/engagement、store/slices/engagement-pool、5 generated DTO）；`ConversationTabs.tsx` 重写为纯标签条；conversation slice 去 engagementRole/workerMeta；解绑 store（index/slices/index）+ 修 lib/api/index.ts + conversation-db-sync。**保留** StageRunView/Card/Detail + harness-handlers + session stageRun slice。验证：`just check-fe`(biome+tsc) exit 0 + `just test-fe` exit 0。
- **S4 全量门禁（已验证）**：`cargo fmt` + `cargo clippy --workspace -- -D warnings` exit 0 **零告警（全 workspace）** + `cargo nextest --workspace` **3269 passed / 7 skipped** + check-fe/test-fe 绿 = 完整 `just check` 全绿。
- **改动面**：47 项（删 25 文件 / 改 ~18 / 1 重命名 scheduler→stage_run / 1 新增 fleet.rs）。顺带：`golish-pentest-app/.../manage_organizations.rs` 仅被 `cargo fmt` 规范化（多行 assert!，原有 fmt 漂移，`lint-rust` 的 fmt --check 门禁必需，非业务改动）；`backend/enscan.gob` 的删除是会话开始前既有状态，非本次引入。
- **commit 记录**：未 commit（等用户）。改动在 branch `feat/stage-run-fanout`。
- **feature_list 更新**：新增 `engagement-overview-fleet-removal-2026-06-14`(passing，权威记录)；`engagement-fleet-scheduler-convergence-2026-06-14` + `engagement-phase-bc-worker-pool-2026-06-13` 标「⚠已移除」→passing；`engagement-phase-a-scoping-2026-06-13` 标「部分移除」（总览读模型 snapshot/contract/query/weakness+4 DTO 删除；scoping 工具链 + engagement_truth 保留）。
- **风险 / follow-up**：① `stage_run_call.rs` / `selection_apply.rs` 原把 stage_run 工具标为「DEPRECATED（被 run_engagement_fleet 取代）」——engagement fleet 已删，已更正注释为「现为 chat 内多 org 扇出现行路径」。② 未 commit、未 push。③ 活体未跑（建议 GUI 复看：新对话不再冒总览卡 + stage-run 单卡照常）。
- **下一步**：用户决定 commit（建议单独一个：`refactor: remove engagement overview/fleet, make stage-run self-contained`）+ 是否活体复看。
...
- **commit 已完成（4 commit，未 push）**：`f3a7337b`(golish-db 查询层 PR-A/B) → `bc7328dd`(golish-pentest dns_record_add PR-B) → `df69fde0`(harness gate+hook+reflector PR-A/C，含编译耦合的 06-11 `stage_spec.rs`/`finding_verification_check.rs`/`target_intel.json`) → `c1211ad2`(docs+登记)。只 add 本功能 + 紧耦合 harness 文件，**未 add 无关范围外**（toolsconfig/golish-pty/golish-mcp/bootstrap/frontend/CHANGELOG/analysis 仍留工作树）。
- **commit 独立编译性提醒（pre-existing 耦合，非本功能）**：`git stash` 别会话改动后验证发现，HEAD(`b5abf46b`) 的 `golish-pentest/src/handlers/env.rs:33` 有 pre-existing E0384（`conda_installed`/`conda_path` 未 `mut` 重复赋值），由别会话「统一安装代理接管」的**未提交**工作树改动修复。故我的 commit 单独 checkout 时 golish-pentest 编译失败（缺该修复）——这是工作树混入多会话未提交改动的固有结果，**非我的代码问题**（完整工作树 `just precommit` 全绿已证）。若需分支编译自洽，可一并提交别会话的 env.rs 等统一代理改动（属那条 feature 的 scope，未代为提交）。
