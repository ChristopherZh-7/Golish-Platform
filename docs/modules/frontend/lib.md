...
- PreparedAction人工review/decision wrapper及其九个孤立ts-rs产物已随backend command物理删除；`api/attack.ts`不得重新invoke这两个旧命令。Campaign/PreparedAction历史详情继续通过只读Investigation projection展示。
