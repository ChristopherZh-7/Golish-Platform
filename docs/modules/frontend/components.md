...
`AIChatPanel` 中 Thought 和正文连续出现时要读作同一段 assistant narrative：`ThinkingBlock` 自身不要再追加底部 margin，正文紧跟 Thought 时用轻微 compact top spacing，避免外层 segment gap 与 Thought margin 叠加造成“思考”和正文像两块孤立面板。
