...
`BackgroundJobsBadge` 不能只依赖已到达的 job registry；detail 已经能从当前工具或 sub-agent 工具列表看出 backgrounded 数量时，要用 fallback count 显示 `N running`，避免详情页没有任何后台入口。每个 registry row 是可访问的 exact navigation button：main job 打开原 Tool Detail，sub-agent job 打开 exact parent 并聚焦 child tool。`ToolCallDetailView` / `SubAgentDetailView` header 使用该 badge 时必须保留稳定槽位，后台任务出现/消失只能改变内容可见性，不能让 header 宽度和页面布局抖动。原 tool row 的 `BackgroundJobPanel` 显示 running/stopping/terminal、job id、background elapsed、soft deadline、hard deadline 与 last-output age；Stop 请求被 backend 接受后只显示 `Stopping…`，最终状态只能由 terminal event 决定。
