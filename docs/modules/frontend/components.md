...
Controller 运行流只绑定 exact `::lead:<worker_run_id>`；动态子 Agent 使用 exact `::worker:<worker_run_id>` 并作为 Controller tool-call 的子节点出现。StageTeam read model 中为持久化兼容保留的 `aggregatorKind`、`aggregatorRole`、`isAggregator` 等 schema 字段不属于产品展示语义，不得生成卡片、标签或 fallback identity。Unit→Gate→Plan→Barrier/Request→WorkItem→Worker→Output 的安全调试字段放入显式“调度详情”折叠区；Plan hash、epoch、lease、schema、chain、request 等不得默认铺满页面。loading/error/empty 保留；恢复按钮只能记录 `blocked outcome unknown`、复用稳定 request id，并且绝不重放工具。
...
- 历史 partial/error cell 显示为“历史 attempt 失败”，active automatic retry Worker显示“当前 retry”，manual recovery Worker显示“operator recovery”。三者可以同时存在，历史红色不代表当前 retry已经失败，manual recovery也不能被画成普通自动重试。
