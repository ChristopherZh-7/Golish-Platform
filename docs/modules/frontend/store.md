...
`BackgroundRunMeta` 必须挂在原始 main/sub-agent tool row 上，terminal result 覆盖后仍保留；live `BackgroundJob` 另存 exact origin、soft/hard deadline、last output 和 `running|stopping`。全局入口通过 `openBackgroundJobDetail` 打开原 main tool，或先打开 exact parent sub-agent 再聚焦 child request；不得按 tool name 或最新工具猜测。
