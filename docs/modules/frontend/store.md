...
`BackgroundRunMeta` 必须挂在原始 main/sub-agent tool row 上，terminal result 覆盖后仍保留；live `BackgroundJob`另存exact origin、initial yield、`automaticKill:false`、last output和`running|stopping`，不保存soft/hard process deadline。全局入口通过 `openBackgroundJobDetail` 打开原 main tool，或先打开 exact parent sub-agent 再聚焦 child request；不得按 tool name 或最新工具猜测。
