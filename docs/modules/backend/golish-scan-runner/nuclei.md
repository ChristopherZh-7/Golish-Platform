...
> **一句话职责**：只读 Nuclei template selector——用 current-owner fingerprint 从本地 KB 选择安全、去重的 template id 和可追溯 rationale。
...
`poc_match` 加载 current in-scope/project-bound target guard，只读 current-owner
...
| 符号 | 说明 |
|---|---|
| `select_nuclei_templates_for_target` | current-owner fingerprint → 去重 template selection |
...
- 不得调用裸 `fingerprints::list_by_target` 或做 backfill；target move 后旧 project 指纹不能跟随 stable target id。
