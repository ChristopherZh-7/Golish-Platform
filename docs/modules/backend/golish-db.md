...
- 持久化 Investigation pending-evolution 到 successor Analysis Primary 的窄授权：统一 subject fingerprint、deterministic Worker/WorkItem/chain、append-only rearm receipt、exact replay 与 StageTeam coordinator 生命周期围栏。
...
| Hypothesis Registry Plan B schema | 唯一 `20260729000006_hypothesis_registry.sql`：Investigation 五态默认/operation freeze/adoption、append-only revision + 独立current-head CAS、Candidate Gate decision/member creating authority、VerificationContract/VerificationPlan deferred exact sets、Candidate two-wave/Plan A bundle compound census、projection outbox/head/legacy compatibility/compare；不含 Plan C authority，00005不变 |
| Investigation operation rollout repos | `investigation_rollout`只读share lock；`operation_rollout`持有七态joint rank、fresh freeze、fork source继承与相邻adoption receipt。无promotion/setter；hash与stable request在同一创建事务重验 |
| Candidate analysis/registry repos | snapshot/page/artifact、H1/H2 census/reducer与 semantic-summary exactness；`load_candidate_pre_gate_material_on` 在同一 RR snapshot 生成 Gate material，`apply_candidate_gate_pass` 在独立短事务重验后原子写 compiler seal + canonical generation + projection outbox |
| Investigation projection/read repos | outbox projector、whole-record comparison、legacy compatibility、summary/list/detail/Timeline RR readers与 commit-visible NOTIFY + bounded polling worker；只读，不执行 rollout promotion |
...
- `20260813000003_investigation_company_asset_queues.sql` 是 mandatory additive Investigation queue foundation：所有新 Investigation run 必须先有 sealed company queue 与 per-company asset queue，缺 queue 不允许回退 organization-wide runtime。migration把旧 scope snapshot-global ordinal唯一键前向替换成 `(snapshot_id,depth,ordinal)`，让ordinal按depth重置；queue按 `depth,ordinal,organization_id` 冻结，deferred count guards使冻结后的company/asset denominator不能被raw INSERT扩展。CAS events、durable evolution fuel 与 zero-hypothesis fixed-point receipt 都保留 exact operation/scope/org owner tuple；历史 organization-wide rows只为审计保留，不成为新 run fallback。exact partial unique保证每队列最多一个 active company/asset，普通 fixed-point 在 lane backlog authority落地前固定拒绝。
- `20260813000006_investigation_asset_backlog_fixed_point.sql` 打开普通 `consolidating→fixed_point` 的唯一窄口：同lane canonical root/revision非空、每个current head明确 `closed + verified|refuted|invalid`即可，不依赖旧hypothesis fixed-point receipt、Campaign/Wave终态或pending evolution bookkeeping。后续 `00008` 在pending discovery authority安装后forward-replace close guard，要求尚未消费的真实new-hypothesis discovery为零。零hypothesis继续要求独立zero receipt；其余工具执行状态/次数只作审计。
- `20260814000003_investigation_asset_backlog_dynamic_resolution_provenance.sql` forward收紧普通fixed-point：每个current root必须由dynamic-v2 resolved round、Primary resolution、terminal transition/state event与current terminal head组成exact authority链；receipt同事务写immutable per-root members并封存count/set hash，deferred census拒绝缺失、额外或漂移。旧adjudication/server-validator closed head与历史无member receipt只读审计，不能满足新queue close/replay。
- 同一 `00006` 的最终queue推进还原子写resolution-only stage closure publication；每个frozen-company member都exact绑定唯一 `stage_run_unit` / `stage_team_plan`，同事务关闭plan、以publication watermark将unit CAS为`passed`并写`org_stage_completions`，为外层coordinator close/pass token提供server authority。它不复用旧operation-global run closure/publication路径，也不由queue repo越权提前结束整个`stage_runs` execution。
...
- Scoping Company Identity 的 `needs_human` receipt、Human choice与root create是同 operation/execution的 durable authority；若模型漏掉第二次 resolver调用，finalizer transaction可据此追加唯一 `human_selected/confirmed` superseding receipt。pre-freeze exact resume只读重算同一 witness并要求 exact expected org，caller UUID或提交自然语言不能补齐缺失 authority。
...
- `pool.rs::create_pool` 在一条专用 `PgConnection` 上执行 migration、释放该 session 自己持有的 advisory locks 后再做受控 repair/retry；必须调用 SQLx `Migrator::run_direct`。对专用 connection 调通用 `run` 会触发 SQLx 已知的 `Acquire` 生命周期泛化问题，使 `GolishDb::start` future 失去 `Send`，从而在 Tauri background spawn 边界编译失败。
...
- Vuln anonymous-access 的首轮 partial/timeout 只作为 narrowed successor 的输入，不得被 exhausted-residual sealer 提前封口；只有 exact `shape=narrowed` 且达到冻结 attempt ordinal 的同一 WorkItem/Worker/evidence witness 才可落 terminal blocked。历史 blocker code 与当前 typed batch-timeout code均在该 exact fence 后兼容。
...
- projection source batch、redacted source snapshot/blob、entity direct predecessor、change、batch receipt、legacy projection version和comparison sample均append-only；通用 projection spine 的 source/entity id 是 bounded opaque text，兼容表仍在边界解析 legacy UUID。entity/change共同持有 source/projection hash 并由 exact composite FK 绑定；source/projection heads是唯一可变行，并由完整batch/receipt的连续CAS trigger推进。Plan C future entity词汇只存在于closed projection catalog，`verification_capability_assessments`、revision adjudication/terminal-decision表仍不存在。
- focused schema入口：`cargo nextest run -p golish-db --test hypothesis_registry`；projection/read入口另见本卡repository kernel。
...
- `repo::candidate_analysis::validate_candidate_analysis_exact_closure_on` 对每个 subreview/synthesis semantic summary重算canonical hash，并要求 covered input/checklist、observed H1 proposal、typed missed checklist、blocker code及最多64条typed observation与原始leaf/node exact closure一致；hash-only summary不能成为Gate authority。
- production finalization顺序固定为：`load_candidate_pre_gate_material_on` 在单一 RR snapshot 冻结exact closure与DB-clock authority → app运行无I/O pure Gate → `repo::hypothesis_registry::apply_candidate_gate_pass` 在新的短RR事务锁定并重验snapshot/epoch/feed/attempt/coverage/host material，随后同事务创建compiler seal、canonical root/revision/component/contract/plan/generation/transition/disposition/relation和immutable projection outbox。不得在首次Gate前单独pre-seal，也不直接写materialized/legacy projection或推进projection head。
...
## Investigation zero-hypothesis migration（2026-08-12）

`20260812000001_investigation_zero_hypothesis_generation.sql` 是 forward-only additive constraint replacement：允许合法零proposal generation，同时保留proposal/proof/mutation耦合与零action不变量。用户已明确授权本次schema/migration改动；既有row不重写。

`20260812000002_verification_directory_fingerprint_witness.sql` 扩展receipt finalization witness闭集，并窄化扩展terminal commit marker：只有exact directory action、`complete_fingerprint_v1` finalization、committed/sampled receipt和DB重算一致的proof/refutation Oracle可走non-raw路径；既有complete-raw与inconclusive语义不变。

`complete_fingerprint_v1`不保留raw response body，因此receipt继续是`coverage_extent='sampled'`；repository会从exact finalization observation冻结一个`complete` typed input、evidence authority、target epoch、temporal census和`consistent` semantic reconciliation。Tool Truth freshness读取接受这条content-addressed typed snapshot authority或既有raw artifact，绝不通过把sampled receipt伪改为complete绕过freshness。

`20260812000004_candidate_verification_fact_delta_sources.sql` forward-only 扩展 Candidate snapshot source-kind 闭集，仅新增 `verification_fact_deltas`；repo 从 previous generation seal 的 canonical consumption spine 冻结 redacted FactDelta 与 exact outcome/adjudication/terminal/capability/Oracle authority，既有 source rows 不重写。
- `20260812000003` 增加Investigation Verification execution assignment immutable row、CAS head、append-only event/recovery和exact owner/JIT/task-plan/WorkItem guards；`20260812000005`/`00006`增加material FactDelta pending evolution authority与compiler fixed-point response-loss receipt；`20260812000007`只允许exact pending authority重启一个Evolution Analysis Primary；`20260812000008`以forward `CREATE OR REPLACE FUNCTION`修正durable-begin旧trigger中的PostgreSQL别名歧义，不重写历史migration。
- `20260814000004_investigation_dynamic_task_admission.sql` 将新compiler task contract扩为`hypothesis_verification_task.dynamic_v2`，并以insert trigger禁止dynamic task获得历史objective assignment/Campaign reservation；既有`hypothesis_verification_task.v1` member/campaign行由forward read-only triggers保持不可变审计。
- `20260814000006_investigation_asset_primary_execution_rearm.sql` 建立exact exhausted dynamic Asset Primary的首代append-only recovery authority；`20260814000009_investigation_asset_primary_multi_execution_rearm.sql` 将其扩为最多32代的predecessor-linked continuation。每代旧WorkItem/Worker/output不变，fresh successor在新dispatch epoch复用同一message chain并递增Worker generation；open task plan保持原logical dispatch，只以immediate predecessor marker续接，Refiner按同root schedule的base/rearm祖先校验authority；current-authority view仅返回最新leaf，fuel用尽typed fail closed。
- `20260814000008_investigation_generator_atomic_recovery.sql` 前向替换Refiner v2 create/append/seal authority并新增append-only Generator source/adoption receipt。task-plan冻结的Primary dispatch可来自同source schedule/message chain的base或任一applied rearm祖先，但提交必须同时持有current execution head consumer fence；finished `submit_result`、0..N subtask census、Generator event与ledger由短事务exact绑定，retained torn rows只能经server-projected候选和exact census adoption恢复。
- `20260814000010_investigation_dynamic_delegation_census.sql` 让dynamic Refiner的delegation seal只统计distinct direct Worker已完成集合：每个成员必须有Primary直系dispatch、非Primary Worker、`completed|residual` terminal attempt及同hash的nonnull result barrier，final patch的`completed_subtask_ids`必须与DB派生集合双向相等且final active denominator为0；被drop的append-only runnable行仅保留审计。v1仍按全部runnable成员封口。exact dynamic zero Generator可在空census与Primary terminal attempt/同hash `primary_synthesis` event齐全时合法封存零subtask plan；Refiner seal barrier不能冒充Primary synthesis。

## Asset Primary fixed-roster scheduling（2026-08-13）

- `20260813000005_investigation_asset_primary_scheduling.sql` 新增 append-only `investigation_asset_primary_schedules`：exact current asset lane + evolution epoch 决定一个 schedule receipt；Primary message chain 只由 asset lane 派生并跨 epoch 复用，Primary WorkItem/WorkerRun 与四个 role WorkItem 则按 epoch 派生。
- 新 epoch 只能由 building receipt 重开已关闭的 Investigation TeamPlan；同一事务必须 server-seed `[browser,researcher,pentester,adviser]` exact-four `required_for_barrier` WorkItem，Primary 不计入该 denominator。applied 后额外/第五 WorkItem insert 由 DB trigger 拒绝；replay 重验 operation/scope/org/target/context/plan/roster 全身份。
- production 事务入口为 `repo::runtime_memory_tx::ensure_investigation_asset_primary_schedule`；focused lifecycle/schema 入口为 `cargo test -p golish-db --test investigation_asset_primary_scheduling`。
