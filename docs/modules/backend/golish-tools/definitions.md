...
- `query_target_data.exact_origin` 是可选的 Vuln planning selector，schema 明确要求复制 current server-authored work item 的 absolute HTTP(S) target URL；它不代表 authorization，production executor/app bridge/wrapper 仍分别执行 org、operation、manifest 与 send-time exact-set guard。
