...
- legacy `js_collect` 已从 `BridgeToolSelection`、chat/task policy 与 prompt tool table 删除；当前 JS 收集链是 `browser_collect_js_api` 做浏览器/runtime capture，再由 `js_extract_apis` 分析已保存的 JS。新增或修改模式时必须保持 selection、渲染表和实际 registry 三者一致，不能重新暴露已删除的工具名。
