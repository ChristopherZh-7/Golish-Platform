...
- `stage_run_call.rs` 给 specialist objective 的后台任务语义必须与 submit barrier 一致：长扫后台化后不要重跑；`submit_stage_deliverable` 负责等待归因 job 落证据，只有明确卡死时才检查/kill。EAS Prober objective 是 coverage-driven，不是固定流水线：先用 `check_stage_asset_coverage` / `query_target_data` 理解当前资产和缺口，再选择最小有效 batch；禁止对原始 in-scope 大列表直接跑 broad `nmap -sV -iL`，URL 资产进 nmap 前必须归一成 host/IP，SERVICE-FINGERPRINT 只能基于确认开放端口的 host:port 分组。后台等待按 visible wait/check loop 执行：任一 job 完成时先读完成输出和落库 evidence；有 stdout/stderr 进展就继续等；idle 或批次明显过宽时 `check_job`/`kill_job` 并终态收口。
- depth-0 stage orchestrator 会在 active harness stage 里看到只读 coverage/target query 工具（`list_in_scope_targets` / `list_attack_surface_seeds` / `query_target_data` / `check_stage_asset_coverage`）；这些工具既要在 `tool_list` 暴露，也必须在 direct executor 走 `execute_security_analysis_tool`，不能只声明不路由，否则主 agent 会得到 `Unknown tool`。`check_stage_asset_coverage` 还需要透传当前 `harness_stage` / `harness_org_id` / `harness_operation_id`，这样它才能默认按当前阶段和 `stage_started_at` 做提交前缺口预检。coverage-gated specialist objective 必须把它写成提交前 mandatory self-check：`ready_to_submit=false` 时先按 `gap_examples` / `cell_summary` / `next_action` 补洞或终态收口，不要靠 `submit_stage_deliverable` 试错。
...
- `sub_agent_call.rs` 还负责给子 agent 注入非 `ToolRegistry` 工具路由：`list_in_scope_targets` / `list_attack_surface_seeds` / `query_target_data` / `check_stage_asset_coverage` 等 security-analysis read helpers 走 `execute_security_analysis_tool`，graph tools 走 graph executor，避免“工具已暴露但 UnknownTool”。
