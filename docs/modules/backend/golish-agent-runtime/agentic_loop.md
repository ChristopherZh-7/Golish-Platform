...
- `stage_run_call.rs` 的 resume-skip 可以复用 fresh `org_stage_completions`，但只要 `operation_state.current_stage` 等于当前 stage，就必须额外要求 completion 晚于本次 `operation_state.stage_started_at`；旧 session 的 completion 不能把 worker 全部短路，否则本轮没有新 evidence/source coverage，最终 submit 会被 deterministic gate 卡住。
