...
- `sub_agent_call.rs` 会把 `submit_stage_deliverable needs_fix` 的 repair directive 写入 `operation_state.state_blob.agent_run`（同 `agent_path`）：`available_evidence_ids` 场景恢复为 evidence-ref repair-only，`running_background_jobs` 场景恢复为 wait-only；下一次 resume 该 sub-agent 时通过 `initial_submit_repair_mode` 注入 executor，并发一条 SubAgentTextDelta 说明正在恢复修复，不从头扫描。
...
- `stage_run_call.rs` 还会在 per-org gate retry 之间写 `operation_state.state_blob.agent_run`：`GateBlocked` checkpoint 保存上一次 gate feedback、attempt index、worker chain ref 和 last tool ref；同一 `agent_path` 恢复时从 pending correction 继续补洞，不从阶段开头重跑。PASS / exhausted / resume-skip 会清理本 org 的 `agent_run` 槽。
