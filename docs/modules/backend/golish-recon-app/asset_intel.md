...
- `agent_intel.rs` 的普通 enrich/org-company 路径会在首轮 provider survey 后，从新写入的 owned domains/app_domains 抽注册 apex，最多自动补 5 个 domain-keyed provider runs；返回 summary 的 `domainExpansions[]` 供 runtime 写 `source_query_log(target=<apex>)`。显式 `AssetIntelHydrateConfig.domain` 仍只跑 domain-keyed templates，用作 targeted repair/manual supplement，且不会再次递归扩展。
