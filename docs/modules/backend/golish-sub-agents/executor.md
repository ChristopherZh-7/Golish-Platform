...
- `submit_stage_deliverable` 的正常后台 reconciliation 在同一次工具调用内事件等待；sub-agent outer timeout 必须对该工具豁免，不能丢弃 barrier future。只有系统 reconciliation deadline 真正过期才进入 one-shot recovery lock：禁止 `wait_for_background_jobs` 循环和替代扫描，只允许每 job 最多一次 `check_job`、确认卡死后的 `kill_job` 与 resubmit。
