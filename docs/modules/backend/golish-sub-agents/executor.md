...
- active Company Controller coordination turn（`bound_worker_chain.stage_team_leader=Some`）不得使用 generic `submit_result` 结束 executor：返回 `STAGE_TEAM_CONTROLLER_REQUIRES_ROUTER` 的非终态 ToolResult 后继续同一 durable chain，只允许 trusted `stage_team_dispatch_workers` / `stage_team_prepare_final_submission` 把控制权交还 scheduler。closed final-submit turn 的 leader binding 已解除，但 `return_on_first_durable_stage_submission` 是更窄的 server-owned boundary：该回合只暴露 `submit_stage_deliverable`，不暴露 generic `submit_result`、查询、扫描、dispatch 或 delegation，避免模型用合法的 specialist barrier 结束却永远无法持久化 StageDeliverable。
...
- `submit_stage_deliverable` 对managed jobs只做一次bounded reconciliation observation；sub-agent outer timeout必须对该工具豁免，不能丢弃barrier future。观察窗口结束时同一job仍存活，返回activity让AI决定下一步；禁止把quiet或yield结束当成deadline。repair lock下不做`wait_for_background_jobs`无界循环或替代扫描，只允许对同一job做有界`check_job`、基于存活/工作量/输出活动判断后显式`kill_job`，再resubmit。
