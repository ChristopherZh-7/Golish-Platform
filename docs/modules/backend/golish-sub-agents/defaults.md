...
- `attack_analyst` 是 reasoning-only Candidate specialist；`candidate_verifier` 工具面精确为 ordinal wrapper、recent evidence、Attempt submit，无 raw runner/Finding writer/scanner/background control/nested delegation。hardcoded 与 registry builder 必须一致。
