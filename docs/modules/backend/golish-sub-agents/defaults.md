...
- WSTG-ATHN-04 的 fallback prompt 必须先以 `query_target_data(target_id, sections=["endpoints"], exact_origin=<work item target_url>)` 读取完整 operation manifest，再把返回的 eligible id 全集作为 `reviewed_endpoint_ids`；不得从裸 target value 推导 scheme/port，也不得在多 origin 间猜测。
