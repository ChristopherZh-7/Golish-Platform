...
- 后台任务约束按 wrapper contract 区分：Prober 四个 EAS wrapper 与 Vuln Scanner 两个 Nuclei wrapper 都强制 foreground，不暴露/使用 job controls；只有实际仍支持后台 job 的其它阶段工具才显式等待并读取完成 tail。
