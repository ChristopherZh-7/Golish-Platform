...
- `submit_stage_deliverable` 面向模型的最小契约只要求 `stage_id` + `claims`；`evidence_refs`、claim `evidence_ids`、`findings`、`coverage`、`skipped_checks`、`required_checks_done` 为空时应省略，由 server canonicalize 为 `[]`。scoping 这种 evidence-free scope decision 不应强行填内部 `SkipReason` enum；子公司排除等普通 scope 决策写进 `scope_confirmed` summary。
