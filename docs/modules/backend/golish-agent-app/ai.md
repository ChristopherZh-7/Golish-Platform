...
- enumeration coverage 的 `GOLISH-ENUM-JSAPI` pending cell 要建议 `browser_collect_js_api` + `js_collect` + `js_extract_apis`：browser closure 是主路径，`js_collect`/`js_extract_apis` 是静态落地和回填，不应诱导 worker 改用端口/EAS 工具。
