...
- `prepare.rs` 按 `GOLISH_EXECUTION_MENTOR` 给 runtime 注入 `ExecutionMonitor`：默认 hard supervisor（`just dev` 直接启用），`shadow` 只记录 mentor advice，`soft`/`on` 注入 advisor，`off`/`false`/`0` 才关闭。这个默认会增加额外 LLM 调用，改默认值必须有明确产品理由。
