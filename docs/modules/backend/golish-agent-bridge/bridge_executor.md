...
- Plan B Candidate runtime capability由app composition root注入bridge service并逐turn快照；executor只传播同一个Arc，不能按profile/default自行创建或选择Registry authority。production adapter已安装；测试/降级环境若为`None`仍必须封闭拒绝，不得回退旧Candidate执行器。
- executor不解释`AnalysisArtifactsReady`、artifact receipt或semantic-summary hash为PASS；最终仍走app的opaque RR pre-Gate snapshot → pure Gate → DB apply transaction。它不持有Plan C/D、promotion或canonical mutation authority。
