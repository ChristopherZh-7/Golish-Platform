...
- `ToolSelectionConfig::main_agent()` 额外开放的 security-analysis 只读工具要和 execution_mode/static policy、direct executor 一起同步；例如 `check_stage_asset_coverage` 既要能被 active stage 看到，也不能泄漏到普通 lead turn。
