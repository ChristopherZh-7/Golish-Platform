...
- `stage_worklist_status` / `stage_worklist_next` 是 `check_stage_asset_coverage` 同源的 DB-truth worklist view：status 返回 compact preflight + 下一步工具建议，next 返回按 `prefer` 过滤的 asset×technique work items（默认 pending/error，带 suggested_tools/evidence_refs/stage-specific focus）。它们只读，不写状态；worker 必须跑工具后重新查询 worklist，不能靠自然语言把 item 标完成。
