...
- 上述 coverage/worklist 三入口必须调用 operation-aware provider seam；Enumeration 返回的 `eas_transport_excluded_origins` 只来自同 operation 的三次 WhatWeb + independent fixed-probe guarded handoff。它们不再作为 work item/root 下发，但诊断仍告诉 worker精确跳过了哪个 owner/origin；缺 operation 或 handoff校验失败时保留 origin，不能 fail-open 地扩大排除范围。
- `stage_worklist_status` / `stage_worklist_next` 是 `check_stage_asset_coverage` 同源的 DB-truth worklist view：status 返回 compact preflight + 下一步工具建议，next 返回按 `prefer` 过滤的 asset×technique work items（默认 pending/error/partial，带 suggested_tools/evidence_refs/stage-specific focus 和可选 `details` / `recommended_args`）。Enumeration snapshot 只含 `exact_web_origin=true` 的 canonical HTTP(S) origin；rootless alive host、unknown TCP service 与无法确定 scheme/port 的 target 从 total/done/pending/gap 全部排除，不能显示成 `missing_exact_web_origin` 或伪 done。`work_item_id` 包含 `target_id + origin value + technique`，同 target 的 HTTP/HTTPS/非默认端口不会相互覆盖。Enumeration 的 found/empty 只认 current-run fresh exact-origin `technique_outcomes`；partial/error 一律未完成，business rows 与自然语言不能关闭 work item。blocked 只认 current-target evidence-backed 的 preflight 四轴、route DIR recovery exhaustion、browser JS/JSAPI/PARAM recovery exhaustion；worklist 文案必须提示普通 partial/error 继续恢复，但 producer 已持久化 `blocked + recovery_exhausted=true` 的 owned cell 不再重试。Enumeration 与 Vuln 三个 worklist/preflight 入口缺 active evidence run/session 或当前 stage cutoff 时直接 fail closed，不读 latest outcome。EAS 输出会额外写明工具边界：domain/URL LIVENESS 用 httpx，IP/CIDR PORT 优先 naabu/masscan，SERVICE 对确认 open ports 跑 `eas_fingerprint_services` / nmap -sV；若 gap 带 `details.missing_open_ports`，worker 应直接用这些端口补扫。WEB-FINGERPRINT item 原样保留 `details.missing_origins` 和 `recommended_args.target_urls`，worker 必须直接复制这些 `{target_id,target_url}`，不能按端口猜 scheme。WhatWeb 只用于确认 HTTP(S) endpoint 的技术指纹。
- EAS WEB 的 parent asset cell 只是 compact read model；只要 `details.missing_origins` 非空，模型传入的 `checked_empty` / `blocked` / `not_applicable` terminal exception 会逐项进入 `rejected_terminal_exceptions`，不修改 snapshot、也不进入 `coverage_to_submit`。因此 compact/status/next 继续显示 partial/pending exact-origin gap，`ready_to_submit` 与 final exact-origin gate 不会再出现“preview 假绿”。其他 EAS technique 的合法 exception 保持原路径。
- `check_stage_asset_coverage` 接受 terminal exception 前，`reason_kind` 必须用 `StageDeliverable` 的共享 `ReasonKind` enum 反序列化；未知值逐项进入 `rejected_terminal_exceptions`，不得修改 snapshot 或把 `ready_to_submit` 推成 true。合法 `credential_missing`（例如 provider 返回 401）保持可提交路径，preview 不得维护另一套字符串白名单。
- `query_target_data` 必须先有 active `harness_org_id`，再读 `in_scope_targets(Some(org_id))` 做 target ownership guard：只有 `target_id` 出现在当前 org/subtree 的 in-scope rows 才能继续 drill-down；missing org、foreign、unowned、invalid row 或 ownership 查询失败都 fail-closed，不能借只读详情接口跨组织读取 endpoint/JS/coverage 数据。
- `search_knowledge_base` 与 `read_knowledge` 必须共享同一持久 wiki logical-path authority。exact read 优先用 DB provider 读取 search 已索引的同一路径；provider 缺失、读取失败或 exact row 不存在时才回退 bundled wiki filesystem。absolute、`..`、空 path 与 DB 返回 path drift 均拒绝，避免 packaged/runtime resource root 不同导致“search 命中但 read file not found”，也避免把 knowledge read 扩成任意文件读取。
- Vuln endpoint review 可额外传 `exact_origin`，但 executor 只在上述 ownership guard 后把它透传给 operation-aware planning projection。它必须来自当前 work item 的 absolute target URL；该值只是 manifest selector，不跳过 app/DB exact-origin校验，也不替代 anonymous wrapper 的 send-time revalidation。
...
- `vuln_triage` 的 coverage snapshot / worklist 继续生成既有 10 个 WSTG/GOLISH technique cell；若 operation-scoped final-sealed Enumeration handoff 明确给出 `total_assets=0` 与 `assets=[]`，返回 `ready_to_submit=true` + `authoritative_zero_input=true`，允许服务端封存 explicit zero-input Vuln handoff，不伪造 scan cell/evidence。零 cell 但没有这组权威空面结构仍返回 `coverage_denominator_missing=true` 并阻断；snapshot 缺失或查询失败在 provider 层直接报错。该 Vuln 封口不等于 Candidate 已具备 initial no-worker terminalization；后者当前仍对 empty evidence/manifest fail closed，需独立补 runtime-memory compound close。九个 WSTG cell 建议 `vuln.nuclei_general` / `vuln_nuclei_general`，`GOLISH-NDAY` 只建议 `vuln.nuclei_fingerprint_targeted` / `vuln_nuclei_fingerprint_targeted`。两者只接 singular `target_id + target_url + techniques` 并 foreground 自落 evidence/outcome；raw Nuclei 和后台 job 控制不进入 Vuln worker 工具面，PASS/BLOCK 仍只看 DB/gate truth。
