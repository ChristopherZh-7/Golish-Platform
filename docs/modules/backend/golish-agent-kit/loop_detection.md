...
> **一句话职责**：循环检测与执行监督——硬性 loop guard 防无限循环/失控，`ExecutionMonitor` 负责 PentAGI-style mentor 触发（shadow/soft）。
...
- 改执行 mentor 触发阈值、shadow/soft 注入模式时
...
`ExecutionMonitor` 是较软的执行监督器：跟踪同工具重复/总工具调用阈值，触发 mentor LLM 评审。`GOLISH_EXECUTION_MENTOR=shadow` 只记录建议，`soft`/`on` 才把建议追加到工具响应；默认 off，避免无意增加模型调用成本。

## 公开接口

| 符号 | 说明 |
|---|---|
| `ExecutionMonitor` / `ExecutionMonitorMode` | mentor 触发监视器；`Shadow` 只记录、`SoftInject` 注入建议 |
