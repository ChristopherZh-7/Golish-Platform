...
- `stage_refiner` 是 gate/submit 失败后的唯一修复建议来源：gate 仍决定 PASS/BLOCK，StageRefiner 只查询/汇总 gate+DB-backed context，生成下一步 `RepairDirective`，不能伪造 StageDeliverable。EAS coverage-gap repair 必须 batch-first，把同 technique 的 sibling gaps 合成少量 `pentest_run`（httpx stdin/input_lines；naabu/masscan/nmap/whatweb/gowitness 用 `{{input_file}}` + `input_lines`）再提交，避免被 gap_examples 带成一资产一调用；submit 同时报 evidence_ref 和 coverage gap 时优先修 coverage gap。SERVICE-FINGERPRINT repair 的 suggested tool 只给 canonical `nmap`，参数放在 command hint；先读已确认 open ports，对 host:port 分组跑 `nmap -Pn -sV`，WhatWeb 只用于已确认 HTTP(S) endpoint 的技术指纹，不能作为 DNS/MySQL/SSH 等通用 service fallback。无开放端口、不可解析或批次过宽的资产要用 blocked/not_applicable/checked_empty 终态说明收口，不能 broad service sweep 后循环等待。
...
- `prompts::stage_charter` 的 submit 示例不要把内部 canonical empty arrays 伪装成模型必填字段：`submit_stage_deliverable` 由后端补齐省略的 `evidence_refs` / `findings` / `coverage` / `skipped_checks` / `required_checks_done`，scoping evidence-free claim 可以省略 `evidence_ids`，普通 scope exclusion 不应被描述成 skipped check。
