...
- Active recon 入口不是普通 HITL：direct EAS stage request 只做 trusted-target check，不会把 provider discovery 升格为授权。TargetIntel→EAS 使用 operation-bound `scope_review`：如果本阶段刷新了 `source=asset_intel` 候选，必须展示 exact org 当前完整 active target denominator，人工只能确认原样非空子集；确认事务把 selected provider rows 升为 `customer_provided/in`、其余 presented rows 改为 `out`，再写 `operation_state.state_blob.active_recon_target_scope` 与 audit。该确认就是 active-scan boundary，成功后直接进 EAS，不再弹第二个 generic phase approval。typed fresh launch 的 `ConfirmedOrganizationIntake` 投影 `Some(false)`，历史 target 只有同 operation durable review 与当前 trusted snapshot exact-match 才能解锁；`ConfirmedTargetIntake` 投影 `Some(true)` 仍需读 DB truth。缺 org、无候选、空/编辑/新增/重复 response、Skip/timeout、candidate drift、DB error 均 fail closed 并发出 `waiting_target_scope` + `ACTIVE_RECON_TRUSTED_TARGET_REQUIRED`。
