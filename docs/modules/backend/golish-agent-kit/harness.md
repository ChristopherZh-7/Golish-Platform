...
Investigation 不再拥有 embedded operator tool catalog。验证角色只能通过动态 Tool Manager inventory snapshot 选择本机已安装且 ready 的工具；历史 Campaign/PreparedAction catalog 已物理删除，不能作为 fallback 或 stage-spec 固定目录重新接回。
...
- Candidate 的 TLS 结论仍由 AI 基于 frozen observation/context/evidence 决定，Decision Gate 不硬编码“必为 Candidate”。但 `nuclei_match_v1 + WSTG-CRYP-03` 中 `weak-cipher-suites` / `deprecated-tls` / `self-signed-ssl` / `mismatched-ssl-certificate` 属于 security lead，不能用 `observation_not_exploitable`、`informational`、`low_severity` 或新造同义词直接关闭；AI 可创建低优先级 exact Nuclei replay，也只可用 closed allowlist `duplicate_candidate|evidence_stale|target_out_of_scope|replay_not_safe|context_refuted|observation_invalid` 加 frozen evidence/rationale 终结。issuer/SAN/version/wildcard 默认是 `tls_metadata_only`，但若上下文形成具体假设仍允许 Candidate。Gate 继续只校验一项一决定、冻结 evidence、closed reason、hypothesis/rationale 和 server-derived immutable plan。acceptance batch 还在 durable write 前按 normalized target identity + target + technique + hypothesis 拒绝重复语义 Candidate，返回 `ATTACK_CANDIDATE_DUPLICATE_IDENTITY` 和两条 exact work-item key；AI 必须保留一个 Candidate并以 `duplicate_candidate` 关闭重复项，或提供真正不同的 hypothesis，不能依赖数据库唯一约束才发现冲突。
...
- EAS IP/CIDR 的 bounded port producer若在 exact operation epoch、target manifest 与 server-owned attempt namespace下耗尽，只能由 `source=eas_discover_ports_policy` 的 guarded outcome 把适用的 LIVENESS/PORT/SERVICE-FINGERPRINT 投影为 `blocked` residual。投影必须同时匹配 canonical IP/CIDR class、technique、outcome 与正 evidence id；SERVICE-FINGERPRINT 只接受 exact IP，不接受 CIDR。该 producer-owned blocked 可关闭 Gate cell并保留未完成残差，但不等于 found/checked-empty，也不能由模型 coverage row、foreign source、domain value或错 evidence id伪造。
...
- `hypothesis_registry::rollout`中的孤儿`CampaignRoute/select_campaign_route/PersistedOperationContractSnapshot`已删除；operation rollout的其他Candidate authority freeze与mutation helpers仍保留。`harness::verification_campaign::ComponentOracleOutcomeV1`仍被真实Nuclei Oracle复用，本次不删除该领域类型。
