...
- `enumeration` 的 per-org gate 资产轴会在进入 `coverage_complete` 前优先收敛到 EAS 已有 `GOLISH-EAS-LIVENESS` found truth 的 web-capable target（domain/url）。如果没有任何 EAS live truth，则保持原 in-scope 资产轴 fail-safe，避免空 worklist 造成假通过。这个口径必须和 `ai_get_stage_asset_coverage` / `check_stage_asset_coverage` 保持一致。
- `enumeration.allowed_tool_types` 只允许内容枚举类工具：`recon/crawler`、`web/fuzzer`、`web/param`。不要把 `recon/http` 加回 enumeration；`httpx` / `whatweb` / `curl` / `wget` 属于 EAS 探活/指纹，不是 JSAPI/DIR/PARAM repair fallback。
...
- `tool_taxonomy.rs` 把 Golish direct enumeration tools（`browser_collect_js_api` / `js_collect` / `js_extract_apis` / `route_probe_paths`）也视为 scan taxonomy 成员，分别归到 `recon/crawler` 与 `web/fuzzer`；它们不是 control-plane 工具，stage whitelist 应该能允许/拒绝它们。`whatweb` 仍归 `recon/http`，因此 EAS 可用、enumeration 不可用。
