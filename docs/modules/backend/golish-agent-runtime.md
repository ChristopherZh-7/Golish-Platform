...
- unified Investigation 的 Analysis Generator 在重新调用模型前先读取 exact current-authority Primary 的 finished `submit_result`。零个旧 member 时由 repository 原子物化 Generator event + 全部 subtask + Refiner ledger + source receipt；已有孤儿 member 时只采纳按 server 顺序最早、且完整 member census 精确匹配的 canonical source。等价 response-loss 重复可重放，不同 canonical source 同时匹配、invalid-only pending、foreign asset 或 census drift 均 fail closed；确定性的 unified repository authority/infrastructure failure 还会返回 `runtime_control.kind=halt_current_request`，避免同一外层请求自动耗尽 durable Primary。
