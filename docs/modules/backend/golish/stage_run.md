...
| `--stage-run-campaign-authority <absolute-json>` | exact Investigation resume 专用显式 local-operator authority packet；retained-PG 与本机 application DB 共用同一合同，必须同时提供 exact session/task/operation/org/stage selectors，只能按 expected generation/row-version 释放 safety hold 或裁决 exact Prepared Action，`--auto-approve` 不参与此权限路径 |
...
- `--controlled-fixture` fresh run 若命中 held-by-default Campaign safety hold，runner会安全停在 typed halt；必须用上述 exact authority packet显式释放并在同一 retained operation resume。成功闭环要求同一 operation 的八阶段 terminal DB truth，不能把另起 fresh run 或 `--auto-approve` 当作授权证明。
- 本机 application DB 的既有 operation 也可能命中同一个 held-by-default Campaign 边界。此时只能在 operation advisory claim 与完整 expected identity 下读取 regular-file packet；packet 继续经 local-operator principal、generation/row-version CAS 与 append-only receipt 落账，不能新增 Tauri/model入口，也不能从 `--auto-approve` 推导权限。
