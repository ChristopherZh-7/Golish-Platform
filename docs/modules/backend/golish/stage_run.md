...
| `--stage-run-campaign-authority <absolute-json>` | retained-PG Investigation resume 专用显式 authority packet；必须同时提供 exact session/task/operation/org/stage selectors，只能按 expected generation/row-version 释放 safety hold 或裁决 exact Prepared Action，`--auto-approve` 不参与此权限路径 |
