...
- `background_jobs` 只有在 direct child 已 wait **且 stdout/stderr 两个 pump 都 EOF** 后才把 job 标 terminal/广播 completion；terminal 仍是 `unreconciled`，必须等 bridge listener 完成 evidence、structured outcomes、agent note 与 UI terminal event 后调用 `mark_reconciled`，stage closeout 的 `wait_for_session_reconciled` 才会被事件唤醒。pump read/join panic 或孙进程继承 pipe 超过 1s drain timeout 时必须 abort pump、写 `output drain incomplete` 诊断并 fail-closed，绝不能把尚未读入 state 的字节伪装成 clean empty。`pty_interactive` 随后对完整 retained stdout/stderr 返回 `*_truncated` 与 `*_original_bytes` 元数据；安全 producer 必须把截断视为非终态，不能把未保留的 batch tail 推断成 checked-empty。
...
- 正常 closeout 不再让模型循环调用 `wait_for_background_jobs` / `check_job`；`submit_stage_deliverable` 在同一次调用内等待 manager 的 reconciliation event。`wait_for_background_jobs` 保留为人工/异常恢复工具；系统 deadline 真正过期时最多一次 `check_job`，只有确认卡死才 `kill_job`。`JobState` 保留共享 completion claim 与 unreconciled terminal，broadcast lag 时 listener 可重放且不重复副作用；prune 不得删除尚未 reconcile 的 attributed terminal job。
