...
export const ContextUsageBadge = memo(function ContextUsageBadge({
  utilization,
  usedTokens,
  maxTokens,
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          title={`Context: ${Math.round(utilization * 100)}% used`}
          className={cn(
...
export const LangfuseBadge = memo(function LangfuseBadge({
  telemetryStats,
  onRefresh,
}: LangfuseBadgeProps) {
  return (
...
export const McpServersBadge = memo(function McpServersBadge({
  sessionId,
  sessionWorkingDirectory,
}: McpServersBadgeProps) {
  ...
  useEffect(() => {
    ...
    const loadMcpData = async () => {
      try {
        ...
        if (sessionId) {
          try {
            setMcpTools(await mcp.listTools());
          } catch {
            setMcpTools([]);
          }
        }
                  ...
                  {providerEntries.map(([name, stats]) => (
                    ...
                    >
                      ...
                      <span
                        ...
                        title={
                          stats.last_sent_at
                            ? new Date(Number(stats.last_sent_at)).toLocaleString()
                            : "—"
                        }
                      >
                        {stats.last_sent_at
                          ? formatRelativeTime(Number(stats.last_sent_at), "localeDate")
                          : "—"}
                      </span>
                      <span
                        className="text-right tabular-nums"
                        title={
                          stats.last_received_at
                            ? new Date(Number(stats.last_received_at)).toLocaleString()
                            : "—"
                        }
                      >
                        {stats.last_received_at
                          ? formatRelativeTime(Number(stats.last_received_at), "localeDate")
                          : "—"}
                      </span>
                    </div>
                  ))}
