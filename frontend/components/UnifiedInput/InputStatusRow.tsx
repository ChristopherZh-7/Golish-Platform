...
export const InputStatusRow = memo(function InputStatusRow({ sessionId }: InputStatusRowProps) {
  ...
  const {
    ...
    const sNv = providerVisibility.nvidia && providerEnabled.nvidia;
    return {
      showVertexAi: sVA,
      ...
      showNvidia: sNv,
      hasVisibleProviders:
        sVA || sVG || sOR || sOA || sAn || sOl || sGe || sGr || sXa || sZs || sNv,
    };
                    ...
                    {showNvidia && (
                      <>
                        {(showVertexAi ||
                          ...
                          showGroq ||
                          showXai ||
                          showZaiSdk) && <DropdownMenuSeparator />}
                        <div className="px-2 py-1 text-[10px] text-muted-foreground uppercase tracking-wide">
                          NVIDIA NIM
                        </div>
                        {(getProviderGroup("nvidia")?.models ?? []).map((m) => (
                          <DropdownMenuItem
                            key={m.id}
                            onClick={() => handleModelSelect(m.id, "nvidia")}
                            className={cn(
                              "text-xs cursor-pointer",
                              model === m.id && provider === "nvidia"
                                ? "text-accent bg-[var(--accent-dim)]"
                                : "text-foreground hover:text-accent"
                            )}
                          >
