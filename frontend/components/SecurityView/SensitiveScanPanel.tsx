...
export function SensitiveScanPanel() {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    setResults((prev) =>
      prev.map((r) => (selectedIds.has(r.id) ? { ...r, isConfirmed: true } : r))
    );
    setSelectedIds(new Set());
  }, [selectedIds]);
    ...
    async (onlyNew: boolean) => {
      ...
      try {
        ...
        const apiKey =
          ...
          "";
        const provider = settings.ai?.anthropic?.api_key
          ? "anthropic"
          : settings.ai?.openai?.api_key
            ? "openai"
            : settings.ai?.openrouter?.api_key
...
${items}`;
        ...
        if (jsonStart >= 0 && jsonEnd > jsonStart) {
          ...
          if (Array.isArray(verdicts)) {
            const applied = await securityApi.sensitiveScanAiAnalyze(projectPath);
  ...
  const dirWordlists = wordlists.filter(
    (w) => w.category === "directories" || w.category === "fuzz" || w.category === "custom"
  );
      ...
      <div className="px-4 py-3 border-b border-border/10 space-y-2 flex-shrink-0">
        ...
        {running && progress && (
          <div className="space-y-1">
            ...
            {progress.currentUrl && (
              <div className="text-[8px] text-muted-foreground/20 font-mono truncate">
                {progress.currentUrl}
              </div>
            )}
          </div>
        ...
        {results.length === 0 && !running ? (
          <div className="h-full flex flex-col items-center justify-center gap-2 text-muted-foreground/20">
            ...
            <p className="text-[9px] text-muted-foreground/15 max-w-[260px] text-center">
              Probes each directory level from sitemap for sensitive files (.env, .git, backups,
              configs, etc.)
            </p>
          </div>
        ) : (
              ...
              {results.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className={cn(
                      "flex items-center gap-2 px-4 py-2 hover:bg-white/[0.02] transition-colors cursor-pointer",
                  ...
                  >
                    ...
                    />
                    <span className={cn("text-[10px] font-mono w-8 text-center", statusColor)}>
                      {r.statusCode}
                    </span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[8px] font-mono text-muted-foreground/25 truncate">
                        {r.baseUrl}
                      </div>
                    </div>
                    <span className="text-[8px] text-muted-foreground/20">
                      {r.contentLength > 0 ? `${r.contentLength}b` : ""}
                    </span>
                    {r.isConfirmed && (
                    ...
                    {r.aiVerdict && (
                      <span
                        className={cn(
                          ...
                          r.aiVerdict === "true_positive"
                            ? "bg-red-500/15 text-red-400"
                            : r.aiVerdict === "false_positive"
                              ? "bg-zinc-500/10 text-zinc-400"
                              : "bg-yellow-500/10 text-yellow-400"
                        )}
                      >
                        {r.aiVerdict.replace("_", " ")}
                      </span>
                    )}
                  </div>
                );
