...
import {
  Check, Crosshair, Globe, Loader2, Pause, Play, Plus,
  Square, ShieldCheck, Trash2, X, Zap,
} from "lucide-react";
...
import { PolicyDropdown, CredentialDropdown } from "./ScanConfig";
import { ScanDetailTabs, AlertCard, riskColor, riskIcon } from "./ScanResults";
...
export function ScannerPanel({ initialUrl, initialBatchUrls, onUrlConsumed }: { initialUrl?: string | null; initialBatchUrls?: string[]; onUrlConsumed?: () => void }) {
  ...
  const {
    endpoints, selectedUrl, setSelectedUrl,
    selectedEndpoint: sel, scanning, totalAlerts,
    completedCount, scanningCount, pausedCount, queuedCount,
    addEndpoint, removeEndpoint,
    scanSelected: handleScanSelected, scanAll: handleScanAll,
    stopAll: handleStopAll, pauseAll: handlePauseAll,
    resumeAll: handleResumeAll, clearCompleted: handleClearCompleted,
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 py-1.5 border-b border-border/10 flex-shrink-0">
        <CredentialDropdown value={selectedCredential} onChange={setSelectedCredential} open={showCredSelector} onToggle={() => setShowCredSelector(!showCredSelector)} entries={vaultEntries} />
        <span className="w-px h-3 bg-border/20" />
        <PolicyDropdown value={selectedPolicy} onChange={setSelectedPolicy} options={[{ value: "", label: t("security.defaultPolicy", "Default Policy (all)") }, ...scanPolicies.map((p) => ({ value: p, label: p }))]} />
        <button
          ...
          onClick={() => {
            if (!showPlugins && scannerRules.length === 0) {
              securityApi.zapGetScanners().then((r) => setScannerRules(Array.isArray(r) ? r : [])).catch(() => {});
            }
        ...
        <span className="text-[9px] text-muted-foreground/30">{endpoints.length} {t("security.endpoints")} · {totalAlerts} {t("security.alertsTotal")}</span>
        {completedCount > 0 && (
          <button type="button" onClick={handleClearCompleted} className="text-[9px] text-muted-foreground/30 hover:text-foreground transition-colors">{t("security.clearCompleted", "Clear Done")}</button>
        )}
        {endpoints.length > 0 && (
          <button type="button" onClick={handleClearAll} className="flex items-center gap-1 text-[9px] text-muted-foreground/30 hover:text-destructive transition-colors"><Trash2 className="w-2.5 h-2.5" /> {t("security.clearAll", "Clear All")}</button>
        )}
      </div>
      ...
      {showPlugins && (
        <div className="border-b border-border/10 bg-muted/5 max-h-[300px] flex flex-col">
          <div className="flex items-center gap-2 px-4 py-1.5 border-b border-border/5 flex-shrink-0">
            ...
            <span className="text-[9px] text-muted-foreground/30">{scannerRules.filter((r) => r.enabled).length}/{scannerRules.length} {t("security.enabled", "enabled")}</span>
            <div className="flex-1" />
          ...
          <div className="flex-1 overflow-y-auto">
            {endpoints.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground/20">
                ...
                <p className="text-[10px] text-muted-foreground/15 max-w-[220px] text-center">{t("security.addTargetsHint", "Add URLs above or right-click endpoints in HTTP History / Site Map → Active Scan")}</p>
              </div>
            ) : (
              <div className="divide-y divide-border/5">
                {[...endpoints].sort((a, b) => b.addedAt - a.addedAt).map((ep) => {
                  ...
                  return (
                    <div key={ep.url} onClick={() => setSelectedUrl(ep.url)} className={cn("px-3 py-2.5 cursor-pointer transition-colors group", selectedUrl === ep.url ? "bg-accent/8" : "hover:bg-[var(--bg-hover)]/30")}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {(ep.status === "scanning" || ep.status === "spidering" || ep.status === "paused") && (
                          <>
                            ...
                            {ep.messageCount != null && ep.messageCount > 0 && <span className="text-[8px] text-muted-foreground/25">{ep.messageCount} req</span>}
                          </>
                        )}
                        <div className="flex-1" />
                        <button type="button" onClick={(e) => { e.stopPropagation(); handleRemoveEndpoint(ep.url); }} className="p-0.5 rounded text-muted-foreground/0 group-hover:text-muted-foreground/30 hover:!text-destructive transition-colors"><X className="w-3 h-3" /></button>
                      </div>
          ...
          {sel ? (
            <>
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/10 flex-shrink-0">
                ...
                {sel.status === "queued" && (
                  <button type="button" onClick={handleScanSelected} className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-medium bg-orange-500/10 text-orange-400 hover:bg-orange-500/20 transition-colors">
              ...
              {sel.status === "complete" && (sel.alerts.length > 0 || scanLogs.length > 0) ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-muted-foreground/30">
                  ...
                  <div className="text-center space-y-1.5">
                    ...
                    <div className="flex items-center gap-3 justify-center">
                      ...
                      {sel.messageCount != null && sel.messageCount > 0 && <span className="text-[11px] text-muted-foreground/40">{sel.messageCount} {t("security.requestsSent", "requests sent")}</span>}
                      {sel.alerts.length > 0 && <span className="text-[11px] text-red-400/60">{sel.alerts.length} alerts</span>}
                    </div>
                  </div>
                </div>
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground/20">
                  ...
                  <p className="text-[13px] font-medium">{sel.status === "complete" ? t("security.scanNoResults") : t("security.scanHint")}</p>
                </div>
              )}
            </>
          ) : (
            <div className="h-full flex flex-col items-center justify-center gap-3 text-muted-foreground/20">
              <ShieldCheck className="w-12 h-12" />
              <p className="text-[13px] font-medium">{endpoints.length > 0 ? t("security.selectEndpoint", "Select an endpoint to view results") : t("security.scanHint")}</p>
            </div>
          )}
        </div>
      </div>
