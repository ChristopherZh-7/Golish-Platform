...
import { StageTeamWorkspaceView } from "./StageTeamWorkspaceView";
...
export interface StageTeamRunViewProps {
  ...
  /** Live Agent activity from the current chat session; never synthesized from durable status. */
...
function isPrimaryLeader(item: StageTeamWorkItem): boolean {
  return item.stableKey === "leader:primary";
}

function applicationModelRoleLabel(item: StageTeamWorkItem): string | null {
  if (item.role === "application_model_controller") return "Controller";
  if (item.role === "application_model_worker") return "Modeler";
  if (item.role === "application_model_synthesizer") return "Synthesizer";
  return null;
}

function companyControllerState(
  item: StageTeamWorkItem,
  ...
  if (item.status === "running" || item.status === "claimed") {
    return {
      label: applicationModel ? "模型 Controller 运行中" : "Controller 运行中",
      status: "running",
...
const LIVE_STAGE_TEAM_STATUSES = new Set([
  "queued",
  "running",
  "claimed",
  "retry_pending",
...
function stageTeamReadModelIsTerminal(model: StageTeamReadModel): boolean {
  if (
    ["completed", "failed", "blocked", "cancelled", "superseded"].includes(model.executionStatus)
  ) {
    return true;
  }
  const workItems = model.units.flatMap((unit) => unit.plan?.workItems ?? []);
  return (
    workItems.length > 0 && workItems.every((item) => !LIVE_STAGE_TEAM_STATUSES.has(item.status))
  );
}
...
function ApplicationModelControllerPreparing() {
  return (
    <div
      role="status"
      className="mt-2 flex items-center gap-2 rounded border border-cyan-500/25 bg-cyan-500/[0.04] px-2.5 py-2 text-[11px] text-cyan-200"
    >
      <Loader2 className="h-3.5 w-3.5 animate-spin" />
      <span>正在准备 Application Model Controller…</span>
    </div>
  );
}
...
export function StageTeamRunView({
  ...
  const shouldPollStageTeam = Boolean(model && !stageTeamReadModelIsTerminal(model));

  useEffect(() => {
    if (!shouldPollStageTeam) return;
    const timer = window.setInterval(() => {
      void load();
    }, 1_500);
    return () => window.clearInterval(timer);
  }, [load, shouldPollStageTeam]);

  const summary = useMemo(() => {
    const plans = model?.units.flatMap((unit) => (unit.plan ? [unit.plan] : [])) ?? [];
    const items = plans
      .flatMap((plan) => plan.workItems)
      .filter((item) => !item.isAggregator && !isPrimaryLeader(item));
  ...
  const coverageSummary = useMemo(() => {
    ...
    return {
      ...
      historicalFailed: cells.filter((cell) => ["partial", "error"].includes(cell.state)).length,
    };
  }, [coverageByOrg]);
  const vulnAttemptSummary = useMemo(() => {
    const lanes =
      model?.units.flatMap(
        (unit) =>
          unit.plan?.workItems.flatMap((item) =>
            item.workers.map((worker) => workerAttemptLane(item, worker))
          ) ?? []
      ) ?? [];
    return {
      retries: lanes.filter((lane) => lane === "current retry").length,
      recoveries: lanes.filter((lane) => lane === "operator recovery").length,
    };
  }, [model]);
  const isVulnStage = model?.stageKind === "vuln_triage";
  const isApplicationUnderstanding = model?.stageKind === "application_understanding";
  const schedulerDetailsVisible = false;
  ...
  return (
    <section className="h-full min-h-0 overflow-hidden rounded border border-border/30 bg-muted/10 p-2">
      <StageTeamWorkspaceView
        model={model}
        agentActivities={agentActivities}
        agentRequestIdsByWorker={agentRequestIdsByWorker}
        focusedAgentRequestId={focusedAgentRequestId}
        coverageByOrg={coverageByOrg}
      >
        {model.stageKind === "vuln_triage" && coverageLoading && (
          <div className="inline-flex items-center gap-1 rounded bg-sky-500/10 px-2 py-1 text-[11px] text-sky-300">
            <Loader2 className="h-3 w-3 animate-spin" /> 正在读取扫描进度
          </div>
        )}
        {model.stageKind === "vuln_triage" && coverageError && (
          <div role="alert" className="rounded bg-red-500/10 px-2 py-1 text-[11px] text-red-300">
        ...
        {model.stageKind === "vuln_triage" &&
          ...
          coverageSummary.total === 0 && (
            <div className="rounded bg-violet-500/10 px-2 py-1 text-[11px] font-medium text-violet-200">
              当前没有待扫描项
            </div>
          )}
          ...
          <div className="rounded border border-violet-500/20 bg-violet-500/[0.04] px-2.5 py-2">
            <div className="mb-1.5 flex items-center gap-2 text-[10px]">
              ...
              <span className="text-muted-foreground">
                {coverageSummary.terminal}/{coverageSummary.total} cells 终态 · 剩余{" "}
            ...
            <div className="mt-2 flex flex-wrap gap-1.5 text-[10px]">
              {coverageSummary.historicalFailed > 0 && (
                <span className="rounded bg-red-500/10 px-1.5 py-0.5 text-red-300">
                  历史失败 {coverageSummary.historicalFailed} cells
                </span>
              )}
              {vulnAttemptSummary.retries > 0 && (
                <span className="rounded bg-sky-500/10 px-1.5 py-0.5 text-sky-300">
                  自动重试 {vulnAttemptSummary.retries}
                </span>
              )}
              {vulnAttemptSummary.recoveries > 0 && (
                <span className="rounded bg-amber-500/10 px-1.5 py-0.5 text-amber-300">
                  待人工恢复 {vulnAttemptSummary.recoveries}
                </span>
              )}
            </div>
          </div>
        ...
        {schedulerDetailsVisible && (
          <div className="space-y-2" data-testid="scheduler-diagnostics">
            {model.units.map((unit) => (
              ...
              >
                ...
                {!unit.plan ? (
                  isApplicationUnderstanding ? (
                    <ApplicationModelControllerPreparing />
                  ) : (
                    <UnsupportedLegacyTeamRun />
                  )
                ) : (
                  (() => {
                    const controller = unit.plan.workItems.find(isPrimaryLeader);
                    if (controller) {
                      const children = unit.plan.workItems.filter(
                        (item) => !isPrimaryLeader(item) && !item.isAggregator
                      );
                      ...
                      const unitCells =
                        unitCoverage?.assets.flatMap((asset) => asset.coverage) ?? [];
                      ...
                      const controllerDisplay = isVulnStage
                        ...
                        : companyControllerState(controller, isApplicationUnderstanding);
                      const gateDisplay = isVulnStage
                        ? vulnGateState(unit)
                        : companyGateState(unit);
                      return (
                        <div
                          className={cn(
                ...
                {schedulerDetailsVisible &&
                  (!unit.plan ? (
                    isApplicationUnderstanding ? null : (
                    ...
                    <div className="mt-2 space-y-2">
                      <div className="rounded border border-cyan-500/20 bg-cyan-500/[0.04] p-2 text-[10px]">
                        ...
                        <div className="mt-1 flex flex-wrap gap-2 text-muted-foreground">
                          ...
                          <span>
                            {unit.plan.requestsClosedAt
                              ? "request epoch closed"
                              : "request epoch open"}
                          </span>
                        </div>
                        <div className="mt-1 flex flex-wrap gap-2">
                          <span
                            className={
                              unit.plan.barrier.readyToFinalize
                                ? "text-emerald-300"
                                : "text-amber-300"
                            }
                          >
                          ...
                          <span>
                            ...
                            {unit.plan.barrier.requiredWorkItems}{" "}
                            {isApplicationUnderstanding ? "analysis workers" : "SubAgents"} terminal
                          </span>
                      ...
                      <div className="space-y-1.5">
                        {unit.plan.workItems.map((item) => {
                          const displayedRole = isApplicationUnderstanding
                            ? (applicationModelRoleLabel(item) ?? item.role)
                            : item.role;
                          return (
                            <div
                              key={item.workItemId}
                            ...
                            >
                              <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                <span className="font-medium">{displayedRole}</span>
                                <span className="text-muted-foreground">
                                  {item.kind} · {item.stableKey}
                                </span>
                                {isPrimaryLeader(item) && !isApplicationUnderstanding && (
                              ...
                              <div className="mt-1 flex flex-wrap gap-2 text-muted-foreground">
                                ...
                                {item.maxAttempts != null && (
                                  <span>max attempts {item.maxAttempts}</span>
                                )}
                              </div>
                              ...
                              {item.output && (
                                <div className="mt-1.5 rounded bg-muted/25 px-2 py-1.5 text-muted-foreground">
                                  ...
                                  {" · "}
                                  {countLabel(
                                    item.output.checkedEmptyCellCount,
                                    "checked-empty cell"
                                  )}
