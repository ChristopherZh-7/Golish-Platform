  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
        ...
        <div>
          ...
          <div className="text-[10px] text-foreground/60">
            {entry.published
              ? new Date(entry.published).toLocaleDateString("zh-CN", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1 mt-0.5">
            {entry.affected_products.map((prod, i) => (
              <span
                key={i}
                className="text-[9px] px-1.5 py-0.5 bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded"
              >
                {prod}
              </span>
            ))}
      ...
      <div className="flex items-center gap-3 pt-1 border-t border-border/10">
        ...
        <a
          href={`https://github.com/search?q=${entry.cve_id}&type=repositories`}
          target="_blank"
          rel="noopener noreferrer"
