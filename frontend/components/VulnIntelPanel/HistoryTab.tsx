...
import type { ScanHistoryEntry, VulnLink } from "./types";
  ...
  const resultBadge = (result: ScanHistoryEntry["result"]) => {
    const map: Record<string, { cls: string; label: string }> = {
      vulnerable: {
        ...
        label: t("vulnIntel.vulnerable"),
      },
      not_vulnerable: {
        cls: "text-green-400 bg-green-500/10",
        label: t("vulnIntel.notVulnerable"),
      },
      error: { cls: "text-yellow-400 bg-yellow-500/10", label: t("common.error") },
      pending: { cls: "text-zinc-400 bg-zinc-500/10", label: t("vulnIntel.pending") },
    };
  ...
  if (link.scanHistory.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-6 gap-2 text-muted-foreground/20">
        ...
        <p className="text-[10px]">{t("vulnIntel.noHistory")}</p>
        <p className="text-[9px] text-muted-foreground/15">
          {t(
            "vulnIntel.historyHint",
            "Scan history will appear here when you test targets with this vulnerability's PoC"
          )}
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-1">
      {link.scanHistory.map((entry, i) => (
        <div
          key={`${entry.target}-${entry.date}-${i}`}
          className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-muted/5 transition-colors"
        >
          ...
          {entry.details && (
            <span className="text-[8px] text-muted-foreground/30 max-w-[150px] truncate">
              {entry.details}
            </span>
          )}
        </div>
