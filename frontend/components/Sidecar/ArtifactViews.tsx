...
export function ArtifactsView({ artifacts, selectedArtifact, selectedArtifactData, artifactPreview, onSelectArtifact }: ArtifactsViewProps) {
          ...
          <div className="p-2 space-y-1">
            {artifacts.map((artifact) => (
              <ArtifactListItem
                ...
                onSelect={() => onSelectArtifact(selectedArtifact === artifact.filename ? null : artifact.filename)}
              />
            ))}
          </div>
        </ScrollArea>
...
function ArtifactListItem({ artifact, isSelected, onSelect }: { artifact: Artifact; isSelected: boolean; onSelect: () => void }) {
  return (
    <button
      type="button"
      onClick={onSelect}
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5 truncate">→ {artifact.meta.target}</p>
          <p className="text-[10px] text-muted-foreground">
            Based on {artifact.meta.based_on_patches.length} patch{artifact.meta.based_on_patches.length !== 1 ? "es" : ""}
          </p>
        </div>
      </div>
    </button>
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-3 space-y-3">
        <div>
          <span className="text-[10px] px-1.5 py-0.5 rounded font-medium bg-[var(--ansi-cyan)]/20 text-[var(--ansi-cyan)]">PENDING</span>
        ...
        <div>
          ...
          <code className="text-xs font-mono bg-muted px-2 py-1 rounded block">{artifact.meta.target}</code>
        </div>
        <div>
        ...
        <div>
          ...
          {preview ? (
            <pre className="text-xs font-mono whitespace-pre-wrap bg-muted p-2 rounded overflow-x-auto">{preview}</pre>
          ) : (
