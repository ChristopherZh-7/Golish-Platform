import { BookOpen, FolderPlus, Loader2, Plus, RefreshCw, Search, X } from "lucide-react";
import { useCallback, useEffect, useRef, useState } from "react";
import { useTranslation } from "react-i18next";
import { MarkdownEditor } from "@/components/MarkdownEditor";
import { type WikiEntry, type WikiSearchResult, type WikiStats, wikiApi } from "@/lib/wiki";
...
export function WikiPanel({ initialPath }: { initialPath?: string | null }) {
  ...
  const openFile = useCallback(
    async (path: string, fileName?: string) => {
      if (dirty && activePath) {
        try {
          await wikiApi.write(activePath, content);
        } catch {
          /* ignore */
        }
      }
  ...
  const confirmCreate = useCallback(async () => {
    ...
    try {
      if (creating.type === "folder") {
        ...
        await wikiApi.write(
          filePath,
          name.endsWith(".md") || !name.includes(".") ? `# ${name.replace(/\.\w+$/, "")}\n\n` : ""
        );
        await loadTree();
        setCreating(null);
        setNewName("");
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="w-[260px] flex-shrink-0 flex flex-col border-r border-border/10">
          <div className="px-3 py-2 border-b border-border/8">
            <div className="relative">
              ...
              />
              {searching && (
                <Loader2 className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 animate-spin text-muted-foreground/30" />
              )}
            </div>
          </div>
                ...
                <div className="space-y-0.5">
                  {searchResults.map((r, i) => (
                    ...
                    >
                      ...
                      {r.line > 0 && (
                        <p className="text-[10px] text-muted-foreground/40 mt-0.5 truncate pl-[18px]">
                          {r.content}
                        </p>
                      )}
                    </div>
        ...
        <div className="flex-1 min-w-0 min-h-0 flex flex-col">
          {activePath ? (
            ...
            <WikiDashboard stats={wikiStats} onOpenPage={(path) => openFile(path)} />
          )}
        </div>
      </div>
        ...
        >
          ...
          >
            <p className="text-[13px] text-foreground mb-1">
              {t("wiki.deleteConfirm", { name: deleteTarget.name })}
            </p>
            <p className="text-[11px] text-muted-foreground/50 mb-4">
              {t("wiki.deleteConfirmMsg")}
            </p>
