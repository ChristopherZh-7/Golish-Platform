...
export const OutputMiniDropdown = (props: {
  value: string;
...
export function OutputParserEditor({
  ...
  const update = useCallback(
    (patch: Partial<OutputConfigData>) => {
  ...
  const toggleProduce = useCallback(
    (type: string) => {
  ...
  const updatePattern = useCallback(
    (idx: number, patch: Partial<OutputPattern>) => {
      const patterns = config.patterns.map((p, i) => (i === idx ? { ...p, ...patch } : p));
      update({ patterns });
    },
    [config.patterns, update]
  );
        ...
        <div className="rounded-xl bg-[var(--bg-hover)]/20 overflow-hidden">
          ...
          <div className="p-3 space-y-3">
            <div className="flex items-center gap-3">
              <label className="text-[11px] text-muted-foreground/50 w-20 flex-shrink-0">
                Format
              </label>
              <OutputMiniDropdown
                value={config.format}
              ...
              {config.patterns.map((pattern, idx) => (
                <div key={idx} className="rounded-lg border border-border/10 p-2.5 space-y-2">
                  ...
                  <div className="text-[9px] text-muted-foreground/50 px-1">
                    Fields:{" "}
                    {Object.entries(pattern.fields)
                      .map(([k, v]) => `${k}=${v}`)
                      .join(", ") || "none"}
                  </div>
                  <input
                    value={Object.entries(pattern.fields)
                      .map(([k, v]) => `${k}=${v}`)
                      .join(", ")}
                    onChange={(e) => {
      ...
      <div className="w-[300px] flex-shrink-0 flex flex-col rounded-xl bg-[var(--bg-hover)]/20 overflow-hidden">
        ...
        {testResult && (
          <div className="max-h-[200px] overflow-y-auto px-3 py-2">
            <pre className="text-[9px] font-mono text-emerald-400/70 whitespace-pre-wrap">
              {testResult}
            </pre>
          </div>
        )}
