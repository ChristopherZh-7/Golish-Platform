...
function InlineSelect({
  ...
  return (
    <Select value={value || undefined} onValueChange={onChange}>
      <SelectTrigger
        size="sm"
        className="flex-1 h-7 border-transparent bg-transparent hover:border-border/20 text-[12px] shadow-none px-2 gap-1"
      >
        <SelectValue placeholder={t("common.select")} />
      </SelectTrigger>
      <SelectContent position="popper" className="min-w-[120px]">
        {options.map((o) => (
          <SelectItem key={o.value} value={o.value || "_none"} className="text-[12px]">
            {o.label}
          </SelectItem>
        ))}
      </SelectContent>
...
export const VALID_INSTALL_METHODS = [
  "",
  "github",
  "homebrew",
  "homebrew-cask",
    ...
    <div className="flex flex-col gap-0.5 py-2 px-3 rounded-lg hover:bg-[var(--bg-hover)]/30 transition-colors">
      <div className="flex items-center gap-3">
        ...
        {type === "select" && options ? (
          <InlineSelect
            value={val}
            onChange={(v) => ctx.handleFormChange(field, v === "_none" ? "" : v)}
            options={options}
          />
        ) : (
          <input
...
export function InstallFieldRow({
  label,
  subField,
  placeholder,
  mono,
...
export function ParamsEditor({ ctx }: { ctx: EditorFieldsContext }) {
  ...
  const removeParam = (idx: number) =>
    ctx.handleFormChange(
      "params",
      params.filter((_, i) => i !== idx)
    );
  const addParam = () =>
    ctx.handleFormChange("params", [...params, { label: "", flag: "", type: "string" }]);

  return (
    <div className="px-3">
      {params.length === 0 ? (
        <p className="text-[12px] text-muted-foreground/50 py-3 text-center">
          {t("toolManager.noParams")}
        </p>
      ) : (
        <div className="space-y-1">
          {params.map((p, i) => (
            <div
              key={i}
              className="flex items-center gap-2 py-1.5 group/param rounded-lg hover:bg-[var(--bg-hover)]/30 px-1"
            >
              <input
                value={(p.label as string) || ""}
                onChange={(e) => updateParam(i, "label", e.target.value)}
                placeholder={t("toolManager.label")}
                className="flex-[3] h-7 px-2 text-[12px] rounded-md bg-transparent border border-transparent hover:border-border/20 focus:border-accent/40 text-foreground placeholder:text-muted-foreground/20 outline-none transition-colors"
              />
              <input
                value={(p.flag as string) || ""}
                onChange={(e) => updateParam(i, "flag", e.target.value)}
                placeholder="--flag"
                className="flex-[2] h-7 px-2 text-[11px] font-mono rounded-md bg-transparent border border-transparent hover:border-border/20 focus:border-accent/40 text-foreground placeholder:text-muted-foreground/20 outline-none transition-colors"
              />
              <div className="flex-[1.5]">
                ...
                >
                  <SelectTrigger
                    size="sm"
                    className="h-7 border-transparent bg-transparent hover:border-border/20 text-[11px] shadow-none px-2 gap-1 w-full"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent position="popper" className="min-w-[100px]">
                    {["string", "number", "boolean", "file"].map((tp) => (
                      <SelectItem key={tp} value={tp} className="text-[12px]">
                        {tp}
                      </SelectItem>
                    ))}
                  </SelectContent>
