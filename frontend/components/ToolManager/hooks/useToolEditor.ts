...
export function useToolEditor(
  ...
  const validateToolData = useCallback(
    (data: Record<string, unknown>): string[] => {
      ...
      if (rt && !(VALID_RUNTIMES as readonly string[]).includes(rt)) {
        errors.push(
          t("toolManager.validationRuntime", { value: rt, allowed: VALID_RUNTIMES.join(", ") })
        );
      }
      const lm = ((data.launchMode as string) || "").trim().toLowerCase();
      if (lm && !(VALID_LAUNCH_MODES as readonly string[]).includes(lm)) {
        errors.push(
          t("toolManager.validationLaunchMode", {
            value: lm,
            allowed: VALID_LAUNCH_MODES.join(", "),
          })
        );
      }
      ...
      if (tier && !(VALID_TIERS as readonly string[]).includes(tier)) {
        errors.push(
          t("toolManager.validationTier", { value: tier, allowed: VALID_TIERS.join(", ") })
        );
      }
      ...
      if (install?.method) {
        ...
        if (!(VALID_INSTALL_METHODS as readonly string[]).includes(m)) {
          errors.push(
            t("toolManager.validationInstallMethod", {
              value: m,
              allowed: VALID_INSTALL_METHODS.filter(Boolean).join(", "),
            })
          );
        }
      }
      ...
      if (rt && rt !== "native" && rtVersion) {
        ...
        if (allowed && !allowed.some((v) => v.value === rtVersion)) {
          errors.push(
            t("toolManager.validationRuntimeVersion", {
              value: rtVersion,
              runtime: rt,
              allowed: allowed.map((v) => v.value).join(", "),
            })
          );
        }
      }
      ...
      if (params && Array.isArray(params)) {
        for (let i = 0; i < params.length; i++) {
          ...
          if (pt && !(VALID_PARAM_TYPES as readonly string[]).includes(pt)) {
            errors.push(
              t("toolManager.validationParamType", {
                index: i + 1,
                value: pt,
                allowed: VALID_PARAM_TYPES.join(", "),
              })
            );
          }
        }
  ...
  const handleSwitchMode = useCallback(
    (mode: "form" | "raw" | "skills" | "output") => {
      if (mode === "skills") {
        setEditorMode("skills");
        if (editingTool) {
          listSkills(editingTool.name)
            .then(setSkillsList)
            .catch(() => setSkillsList([]));
        }
        return;
