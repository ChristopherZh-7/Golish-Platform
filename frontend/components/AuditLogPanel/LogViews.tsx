...
export function AgentLogsView() {
  ...
  if (!entries.length)
    return (
      <div className="text-center text-[11px] text-muted-foreground/30 py-12">No agent logs</div>
    );
...
export function TerminalLogsView() {
  ...
  if (!entries.length)
    return (
      <div className="text-center text-[11px] text-muted-foreground/30 py-12">No terminal logs</div>
    );
...
export function SearchLogsView() {
  ...
  if (!entries.length)
    return (
      <div className="text-center text-[11px] text-muted-foreground/30 py-12">No search logs</div>
    );
      ...
      {entries.map((e) => (
        <div key={e.id}>
          ...
          >
            ...
            <span
              className={cn("text-[8px] px-1.5 py-0.5 rounded flex-shrink-0", sevColor(e.severity))}
            >
              {e.severity}
            </span>
          ...
          {expanded.has(e.id) && (
            <div className="ml-8 mb-2 px-3 py-2 rounded-lg bg-[var(--bg-hover)]/20 border border-border/10 space-y-1">
              <div className="text-[9px] text-muted-foreground/30">
                <span className="text-muted-foreground/50">Target:</span> {e.targetId.slice(0, 12)}
                ...
              </div>
              {e.url && (
                <div className="text-[9px] text-muted-foreground/30">
...
export function WikiChangelogsView() {
  ...
  if (!entries.length)
    return (
      <div className="text-center text-[11px] text-muted-foreground/30 py-12">No wiki changes</div>
    );
