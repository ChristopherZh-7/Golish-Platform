...
export function useProviderForm(settings: AiSettings, onChange: (settings: AiSettings) => void) {
  ...
  const updateOpenRouterPref = useCallback(
    <K extends keyof OpenRouterProviderPreferences>(
      field: K,
  ...
  const getShowInSelector = useCallback(
    (providerId: ProviderConfig["id"]): boolean => {
