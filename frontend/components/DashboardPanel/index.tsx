                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(["domain", "ip", "cidr", "url", "wildcard"] as const).map((type) => {
                        ...
                        return (
                          <div
                            key={type}
                            className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-muted/10"
                          >
                            <Icon className="w-3 h-3 text-muted-foreground/40" />
                            <span className="text-[10px] text-muted-foreground/50 capitalize flex-1">
                              {type}
                            </span>
                          ...
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            {Object.entries(derived.findingsByTool)
                              ...
                              .map(([tool, count]) => (
                                <span
                                  key={tool}
                                  className="text-[9px] px-2 py-0.5 rounded-full bg-muted/15 text-muted-foreground/50"
                                >
                                  {tool}{" "}
                        ...
                        {stats.recentActivity.map((entry) => (
                          <div key={entry.id} className="flex items-start gap-2 py-1 text-[10px]">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {entry.details && (
                                <span className="text-muted-foreground/35 ml-1 truncate">
                  ...
                  <div className="rounded-xl bg-muted/8 border border-border/10 p-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] text-muted-foreground/30 ml-auto">
                        {stats.findings.length} total
                      </span>
                    </div>
                    <div className="space-y-1">
                      {stats.findings
                        .sort((a, b) => {
                          const sevIdx = (s: string) =>
                        ...
                        .map((f) => (
                          <div
                            key={f.id}
                            className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-muted/10 transition-colors"
                          >
                            ...
                            {f.tool && (
                              <span className="text-[9px] text-muted-foreground/30 px-1.5 py-0.5 rounded bg-muted/10">
                                {f.tool}
                              </span>
                            )}
                            <span
              ...
              <>
                ...
                <div className="grid grid-cols-4 gap-3">
                  <MetricCard
                    ...
                    detail={
                      aiStats.tokenUsage
                        ? `${fmtNum(aiStats.tokenUsage.total_tokens_in)} in · ${fmtNum(aiStats.tokenUsage.total_tokens_out)} out`
                        : undefined
                    }
                    accent="purple"
                  />
                  <MetricCard
                    ...
                    detail={
                      aiStats.tokenUsage
                        ? `$${aiStats.tokenUsage.total_cost_in.toFixed(4)} in · $${aiStats.tokenUsage.total_cost_out.toFixed(4)} out`
                        : undefined
                    }
                    accent="green"
                  />
                  <MetricCard
                    ...
                    detail={
                      aiStats.toolCallStats.length > 0
                        ? `${aiStats.toolCallStats.length} tools used`
                        : undefined
                    }
                    accent="blue"
                  />
                  <MetricCard
                    ...
                    detail={
                      aiStats.agentUsage.length > 0
                        ? `${aiStats.agentUsage.length} agents active`
                        : undefined
                    }
                    accent="amber"
                  />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {aiStats.toolCallStats.length > 0 && (
                    <div className="rounded-xl bg-muted/8 border border-border/10 p-4 space-y-3">
                      ...
                      <ToolCallChart
                        tools={aiStats.toolCallStats.slice(0, 8)}
                        maxCount={aiStats.toolCallStats[0]?.total_count ?? 1}
                      />
                    </div>
                  )}
                </div>
