...
export function PlanStepIcon({ status, size = "sm" }: { status: string; size?: "sm" | "md" }) {
  ...
  switch (status) {
    case "completed":
      return <CheckCircle2 className={cn(s, "text-green-500 flex-shrink-0")} />;
    case "in_progress":
      return <Loader2 className={cn(s, "text-accent animate-spin flex-shrink-0")} />;
    case "failed":
    case "cancelled":
      return <XCircle className={cn(s, "text-red-400/70 flex-shrink-0")} />;
    default:
      return (
        <div
          className={cn(s, "rounded-full border-[1.5px] border-muted-foreground/25 flex-shrink-0")}
        />
      );
  }
}
