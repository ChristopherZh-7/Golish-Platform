...
const STATUS_CONFIG = {
  ...
  pending: {
    icon: (i: number) => (
      <div key={i} className="w-3.5 h-3.5 rounded-full border-2 border-zinc-600" />
    ),
...
function StepRow({ step, index }: { step: StepDetail; index: number }) {
  ...
  const hasContent =
    !!step.output || (!!step.message && (step.status === "error" || step.status === "skipped"));
    ...
    <div className={cn("rounded-md border transition-all", cfg.border, cfg.bg)}>
      ...
      {step.status === "running" && (
        <div className="border-t border-blue-500/10 mx-2 mb-2">
          {step.output ? (
            <LiveOutput output={step.output} isLive={true} onCopy={copyOutput} copied={copied} />
          ) : (
            <RunningIndicator />
          )}
        </div>
      ...
      {expanded && hasContent && step.status !== "running" && (
        <div className="border-t border-white/[0.06] mx-2 mb-2">
          {step.message && step.status === "skipped" && (
            <div className="px-2 py-2 text-[10px] text-zinc-400 italic">{step.message}</div>
          )}
          {step.output && (
            <LiveOutput output={step.output} isLive={false} onCopy={copyOutput} copied={copied} />
          )}
          ...
          <>
            ...
            {progress && (
              <span className="text-[10px] text-zinc-400 ml-auto">
                ...
                <span className="text-blue-300 font-medium">{progress.tool}</span>
              </span>
            )}
