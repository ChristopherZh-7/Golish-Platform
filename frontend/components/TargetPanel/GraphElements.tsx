                    ...
                    <div key={target.id} className={isMulti ? "pl-3" : ""}>
                      ...
                      {children.length > 0 && (isExpanded || !isMulti) && (
                        <div className="pl-4">
                          ...
                          {children.length > 8 && (
                            <span className="px-1.5 text-[8px] text-muted-foreground/30">
...
export function GraphNodeDetail({
  ...
  const children = useMemo(
    () => targets.filter((t) => t.parent_id === target.id),
    [targets, target.id]
  );
          ...
          <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-xs">
            {target.real_ip && (
              <>
                ...
                <dd className="font-mono text-foreground tabular-nums">{target.real_ip}</dd>
              </>
            )}
            {target.os_info && (
              <>
                <dt className="text-muted-foreground">OS</dt>
                <dd className="text-foreground">{target.os_info}</dd>
              </>
            )}
            {target.webserver && (
              <>
                <dt className="text-muted-foreground">Server</dt>
                <dd className="text-foreground">{target.webserver}</dd>
              </>
            )}
            {target.cdn_waf && (
              <>
                <dt className="text-muted-foreground">CDN/WAF</dt>
                <dd className="text-foreground">{target.cdn_waf}</dd>
              </>
            )}
          </dl>
        )}
          ...
          <div>
            ...
            <ul className="space-y-1">
              {target.ports.map((p) => {
                const dot =
                  p.state === "open"
                    ? "bg-emerald-400 ring-1 ring-emerald-500/40"
                    : p.state === "filtered"
                      ? "bg-amber-400 ring-1 ring-amber-500/40"
                  ...
                  >
                    ...
                    {p.http_status != null && (
                      <span
                        className={cn(
                          "text-[10px] font-mono",
                          p.http_status < 400 ? "text-green-400" : "text-red-400"
                        )}
                      >
                        [{p.http_status}]
                      </span>
