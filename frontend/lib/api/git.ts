...
export async function gitStatus(workingDirectory: string): Promise<GitStatusSummary> {
  return invoke<GitStatusSummary>("git_status", { workingDirectory });
}
...
export async function gitDiffStaged(workingDirectory: string): Promise<string> {
  return invoke<string>("git_diff_staged", { workingDirectory });
}
...
export async function listGitBranches(workingDirectory: string): Promise<string[]> {
  return invoke<string[]>("list_git_branches", { workingDirectory });
}
