...
- Do NOT resolve every discovered subdomain one-by-one. Per-host A-record
  ...
  wasted work and is out of this stage's purpose.
...
- target_intel coverage is adjudicated from DB truth. Once a technique actually RAN
  and its data LANDED (subdomains/asset pairs → `target_assets`, DNS records
  when provider landing supplies them → `dns_records`, ASN/CT/WHOIS →
  `organizations.asns/.certificates/.whois`, OSINT → `organizations.intel`),
  the gate marks that (asset × technique) cell `found` on its
