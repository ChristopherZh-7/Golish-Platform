...
- **SubAgent 策略**：只要运行环境支持，本项目**明确允许并鼓励主动使用 SubAgent**；存在两个及以上可独立推进的子任务时默认优先并行委派，不以模型或 reasoning 等级作为启用条件。
...
### 2.0 SubAgent 主动并行（项目级明确授权）

- 本文件构成项目级预授权：只要当前 agent 运行环境支持 SubAgent / 子 agent / 并行 agent，主 agent 可以按任务需要主动调用，**无需逐次向用户申请许可**；这项授权适用于所有模型与 reasoning 等级，不只适用于 ultra。
- 每轮开工时必须判断任务能否安全拆分。若存在 **2 个及以上相互独立、边界清楚、可并行交付**的子任务且有可用并发槽，默认应使用 SubAgent；典型场景包括并行只读调研、跨模块影响面梳理、相互独立的测试失败分析、代码与文档分开审查、多个定向验证。
- 以下情况可不使用：单文件小改、任务有强顺序依赖、上下文无法安全切分、多个 agent 会修改同一文件或争用同一外部状态、运行环境不支持。不要为了满足形式而制造无收益的委派。
- 委派时优先一次并行发出；每个 SubAgent 只承担一个有明确输入、读写范围、交付物和证据要求的 bounded subtask。避免多个 agent 同时写同一文件；主 agent 负责整合结论、解决冲突、完成最终修改与定向验证。
- SubAgent 不扩大任务 scope 或授权边界：所有子 agent 同样必须遵守本 `AGENTS.md`，不得绕过高风险确认、单一 `in_progress`、定向验证或 evidence 规则，也不得自行把功能宣称为 `passing`。
- 主 agent 必须复核 SubAgent 产出；实际采纳的重要结论、修改和验证证据要写入 `agent-progress.md`。仅有“已委派 / SubAgent 已完成”的自然语言不能作为完成证据。
