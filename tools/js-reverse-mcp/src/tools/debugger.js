    ...
    try {
      ...
      if (startLine !== undefined || endLine !== undefined) {
        ...
        if (content.length > 1000) {
          const lineOffset = lines.slice(0, start).reduce((sum, l) => sum + l.length + 1, 0);
          response.appendResponseLine(
...
export const saveScriptSource = defineTool({
  ...
  schema: {
    ...
    filePath: zod.string().describe("Local file path to save the script source to."),
  },
  handler: async (request, response, context) => {
    const debugger_ = context.debuggerContext;
    if (!debugger_.isEnabled()) {
...
export const searchInSources = defineTool({
  ...
  handler: async (request, response, context) => {
    ...
    const { query, caseSensitive, isRegex, maxResults, maxLineLength, excludeMinified, urlFilter } =
      request.params;
    try {
...
export const removeBreakpoint = defineTool({
  ...
  schema: {
    ...
    url: zod.string().optional().describe("The XHR breakpoint URL pattern to remove."),
  },
  handler: async (request, response, context) => {
    const debugger_ = context.debuggerContext;
    if (!debugger_.isEnabled()) {
      ...
      if (match.lineNumber < lines.length) {
        ...
        if (funcStart >= 0) {
          ...
          if (braceMatch && braceMatch.index !== undefined) {
            columnNumber = funcStart + foundMatch.pattern.length + braceMatch.index + 1;
          } else {
