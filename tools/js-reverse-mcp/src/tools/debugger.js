...
export const saveScriptSource = defineTool({
    ...
    description: 'Saves the full source code of a JavaScript script to a local file. Use this to download complete script sources for offline analysis, especially for large or minified files that are too big to view inline with get_script_source.',
    annotations: {
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            response.appendResponseLine(`Saved script source to ${result.filename} (${source.length} chars).`);
        }
        catch (error) {
...
export const injectScript = defineTool({
    ...
    schema: {
        ...
        identifier: zod
            .string()
            .optional()
            .describe('The identifier of a previously injected script to remove.'),
    },
    handler: async (request, response, context) => {
        const debugger_ = context.debuggerContext;
        if (!debugger_.isEnabled()) {
        ...
        if (!script && !identifier) {
            response.appendResponseLine('Either script (to inject) or identifier (to remove) must be provided.');
            return;
        }
        try {
            ...
            if (identifier) {
                ...
                response.appendResponseLine(`To remove: inject_before_load(identifier: "${id}")`);
            }
        }
        catch (error) {
            response.appendResponseLine(`Error: ${error instanceof Error ? error.message : String(error)}`);
